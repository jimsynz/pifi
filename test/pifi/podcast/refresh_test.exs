defmodule PiFi.Podcast.RefreshTest do
  ...
  alias PiFi.Podcast.Show.RefreshAll
  alias PiFi.Test.Podcasts
  ...
  setup do
    ...
    # sets it up. See `PiFi.Source.enabled?/1`.
    PiFi.Source.enable(PiFi.Source.Podcasts, true)
  ...
  # An episode is a `PiFi.Playback.Item` under the container of the show.
  ...
  describe "the scheduled action" do
    ...
    test "the item of a show goes with the show, and it takes the episodes" do
      ...
      assert Podcast.list_shows!() == []
      assert Ash.count!(PiFi.Playback.Item) == 0
    end
  end
  ...
  describe "what it tells the rest of the firmware" do
    test "a read that succeeded says that the show changed" do
      ...
      assert_receive %Event.Source.Changed{source: PiFi.Source.Podcasts, ref: {:show, id}}
  ...
  describe "the schedule" do
    test "Oban holds a worker for it" do
      assert Code.ensure_loaded?(PiFi.Podcast.Show.Workers.RefreshAll)
    end

    test "Oban holds a worker for the read of one show" do
      assert Code.ensure_loaded?(PiFi.Podcast.Show.Workers.Refresh)
    end
    ...
    test "it reads no feed when the podcast source is out of use" do
      ...
      on_exit(fn ->
        {:ok, setting} =
          PiFi.Settings.fetch(PiFi.Source.enabled_key(PiFi.Source.Podcasts))

        PiFi.Settings.delete!(setting)
      end)
