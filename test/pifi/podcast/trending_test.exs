defmodule PiFi.Podcast.TrendingTest do
  ...
  setup do
    ...
    {:ok, _setting} = PiFi.Settings.put(Index.key_setting(), "THEKEY")
    {:ok, _setting} = PiFi.Settings.put(Index.secret_setting(), "THESECRET")
    :ok
  end
  ...
  test "a device that holds no key marks nothing, and the action reports none" do
    for key <- [Index.key_setting(), Index.secret_setting()] do
      {:ok, setting} = PiFi.Settings.fetch(key)
      PiFi.Settings.delete!(setting)
    end

    assert {:error, _reason} = Trending.run()
    assert {:ok, 0} = PiFi.Podcast.read_trending_shows()
  end
end
