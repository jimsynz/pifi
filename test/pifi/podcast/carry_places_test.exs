defmodule PiFi.Podcast.CarryPlacesTest do
  use PiFi.DataCase, async: false

  alias PiFi.Playback
  alias PiFi.Podcast.CarryPlaces
  ...
  setup do
    ...
    PiFi.Repo.query!("ALTER TABLE podcast_shows ADD COLUMN title TEXT", [], log: false)
  ...
  test "a show that a person subscribed to becomes a marked container" do
    ...
    assert %{shows: 1} = CarryPlaces.run(PiFi.Repo)
  ...
  test "an episode that a person part heard keeps its place" do
    ...
    assert %{episodes: 1} = CarryPlaces.run(PiFi.Repo)
  ...
  test "an episode that a person never touched is left alone" do
    ...
    assert %{episodes: 0} = CarryPlaces.run(PiFi.Repo)
  ...
  test "a device with no old tables carries nothing" do
    PiFi.Repo.query!("DROP TABLE podcast_episodes", [], log: false)

    assert %{shows: 0, episodes: 0} = CarryPlaces.run(PiFi.Repo)
  end
  ...
  test "it leaves the old tables as they are" do
    ...
    assert %{rows: [[1]]} =
             PiFi.Repo.query!("SELECT count(*) FROM podcast_episodes", [], log: false)
  end
end
