defmodule PiFi.SwitchOffTest do
  ...
  doctest PiFi.SwitchOff, import: true
  ...
  # `PiFi.Application` starts none of these in the test environment, so a test holds
  # the one that it asks for. See `PiFi.Application.listening_children/0`.
  ...
  describe "the checkpoint of the period" do
    setup do
      ...
      on_exit(fn ->
        for %{id: id} <- :telemetry.list_handlers([:pifi, :repo, :query]),
