defmodule PiFi.AutoSyncTest do
  ...
  doctest PiFi.AutoSync, import: true
  ...
  describe "the jobs that this process runs" do
    test "each one names a key, a title, a source, an action and a period" do
      for job <- AutoSync.jobs() do
        ...
        assert job.source in PiFi.Source.all()
    ...
    # `PiFi.Application` takes these out of the crontab that `AshOban.config/2` builds,
  ...
  describe "the source that a job belongs to" do
    test "a job of a source that a person turned off is never due" do
      ...
      {:ok, :ok} = PiFi.Playback.enable_source(Source.InternetRadio, false)
    ...
    # yet, so this asks after the module is certainly loaded. `PiFi.Source.ready?/1`
  ...
  # A source names what it needs in `c:PiFi.Source.ready?/0`, and a job of a source
