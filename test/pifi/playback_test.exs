defmodule PiFi.PlaybackTest do
  ...
  describe "the domain" do
    test "holds every control, so no page calls the process" do
      # `PiFi.Player` is the process, and each page calls this domain instead.
      for {name, arity} <- [
            state: 0,
            play: 2,
            stop: 0,
        ...
        assert function_exported?(Playback, name, arity),
               "PiFi.Playback.#{name}/#{arity} is absent"
      end
    end
  end
end
