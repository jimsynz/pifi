defmodule PiFi.PeripheralTest do
  @moduledoc """
  What a person turns on, and what the board answers.

  `PiFi.Peripheral.Supervisor` is one process for the whole firmware, and a
  ...
  doctest PiFi.Peripheral, import: true
  ...
  describe "start_enabled/0" do
    ...
    test "hardware that does not answer stops no other start" do
      knows([{Lamp, fault: :no_such_device}])
      PiFi.Settings.put!(Peripheral.enabled_key(Lamp), "true")
  ...
  defp knows(peripherals), do: Application.put_env(:pifi, :peripherals, peripherals)
end
