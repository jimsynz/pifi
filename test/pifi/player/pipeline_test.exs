defmodule PiFi.Player.PipelineTest do
  ...
  defp init(overrides) do
    ...
    Pipeline.handle_init(nil, %{
      ...
      sink: %PiFi.Output.APlaySink{device: "null"},
      parent: self()
    })
  end

  # A track of a library is a file that `PiFi.Player.Download` wrote, so it names a
  # key and a place to begin at. See `PiFi.Player.FileSource`.
