defmodule PiFi.Source.JellyfinTest do
  ...
  describe "the order of the tracks of an album" do
    ...
    # device. See the `place` calculation of `PiFi.Playback.Item`.
  ...
  describe "what the source says about itself" do
    ...
    # track and not the next. `PiFi.Player.Skip.frames/1` holds MP3, AAC and FLAC,
    # which is every codec that `PiFi.Jellyfin.Server` asks a server for.
    #
  ...
  describe "roots/0" do
    ...
    test "the albums of an artist show a release year, and the tracks of an album do not" do
      artist = struct(PiFi.Playback.Item, kind: :container, parent_id: nil)
      album = struct(PiFi.Playback.Item, kind: :container, parent_id: "artist-1")
  ...
  describe "the link flow" do
    ...
    test "a read of the library goes to a job, so a person waits for no network" do
      ...
      assert_enqueued(worker: PiFi.Jellyfin.Sync.Workers.Library)
    end
