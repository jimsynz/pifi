defmodule PiFi.Source.PodcastsTest do
  ...
  setup do
    ...
    on_exit(fn ->
      Application.delete_env(:pifi, Index)
      Application.delete_env(:pifi, Feed)
    end)
  ...
  # The source reads `PiFi.Playback.Item`, so every show of a test needs its item and
  ...
  describe "what the source says about itself" do
    ...
    test "its name in an address comes from the module" do
      assert PiFi.Source.slug(Podcasts) == "podcasts"
      assert {:ok, Podcasts} = PiFi.Source.from_slug("podcasts")
    end

    test "the firmware holds it" do
      assert Podcasts in PiFi.Source.all()
    end
  ...
  # search must ask the index. `PiFiWeb.SearchLive` matches the text against what the
  # query gives.
  describe "search/1" do
    ...
    # they hold. `PiFiWeb.SearchLive` draws a control to choose between the two.
  ...
  describe "resolve" do
    ...
    # `PiFi.Podcast.CarryPlaces` writes an episode that holds the place of a person
  ...
  # calls `PiFi.Playback.set_favourite/1`, and the read of the shows joins to it.
