defmodule PiFi.CacheTest do
  use PiFi.DataCase, async: false

  alias PiFi.Cache

  setup do
    Application.put_env(:pifi, Cache, plug: {Req.Test, Cache}, retry: false)
    on_exit(fn -> Application.delete_env(:pifi, Cache) end)
  ...
  # What `PiFi.Artwork` writes for a thumbnail: an entry that names its source.
  ...
  # `PiFi.Device.Monitor` is the one subscriber, and it is a target module, so this
  # tests the wire that carries the notification to it. Free space moves when the cache
  # writes and when it removes, and a page shows that figure. See `PiFi.Cache.Entry`.
  describe "what the cache tells the rest of the firmware" do
    setup do
      :ok = Phoenix.PubSub.subscribe(PiFi.PubSub, "cache_entry:written")
    end
  ...
  describe "a variant of an entry" do
    ...
    test "an eviction that takes the source takes it as well" do
      ...
      Application.put_env(:pifi, :cache_limit, 50)
  ...
  describe "prune" do
    test "a cache inside its limit loses nothing" do
      ...
      Application.put_env(:pifi, :cache_limit, 1000)
    ...
    test "it removes the least recently used first" do
      ...
      Application.put_env(:pifi, :cache_limit, 900)
    ...
    test "a cache of nothing but entries to keep says that it is over its limit" do
      ...
      Application.put_env(:pifi, :cache_limit, 100)
  ...
  describe "the limit" do
    ...
    test "it grows by what the cache already holds" do
      Application.delete_env(:pifi, :cache_limit)
      Application.put_env(:pifi, :cache_free_bytes, 2 * 1024 * 1024 * 1024)
      on_exit(fn -> Application.delete_env(:pifi, :cache_free_bytes) end)
    ...
    test "a test may name a limit of its own" do
      Application.put_env(:pifi, :cache_limit, 4242)
  ...
  describe "attachments" do
    # A show and an episode are both `PiFi.Playback.Item` now, and `"item"` is the
    ...
    test "a host that goes takes its own join rows", %{show: show} do
      ...
      assert :ok = PiFi.Playback.destroy_item(show)
    ...
    test "an episode that goes takes its own rows and no other", %{show: show} do
      ...
      assert :ok = PiFi.Playback.destroy_item(episode)
