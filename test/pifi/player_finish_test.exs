defmodule PiFi.PlayerFinishTest do
  @moduledoc """
  ...
  defmodule Recorder do
    ...
    `PiFi.Player` writes the place and the played mark on to the item itself, so
    ...
    def live!(live?), do: Application.put_env(:pifi, :recorder_live?, live?)

    @doc "Say where the next resolve begins."
    def begins_at!(position_ms) do
      Application.put_env(:pifi, :recorder_position_ms, position_ms)
    end

    defp live?, do: Application.get_env(:pifi, :recorder_live?, false)

    defp position_ms, do: Application.get_env(:pifi, :recorder_position_ms, 0)
  end
  ...
  setup do
    ...
    Application.put_env(:pifi, :sources, [Recorder])
    Recorder.live!(false)
    ...
    on_exit(fn ->
      ...
      for key <- [:sources, :recorder_live?, :recorder_position_ms] do
        Application.delete_env(:pifi, key)
      end
    end)
