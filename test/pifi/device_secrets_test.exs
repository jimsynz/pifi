defmodule PiFi.DeviceSecretsTest do
  use ExUnit.Case, async: false

  alias PiFi.DeviceSecrets

  setup do
    data_dir = Path.join(System.tmp_dir!(), "pifi-#{System.unique_integer([:positive])}")
    endpoint = Application.get_env(:pifi, PiFiWeb.Endpoint, [])

    on_exit(fn ->
      File.rm_rf!(data_dir)
      Application.put_env(:pifi, PiFiWeb.Endpoint, endpoint)
    end)
  ...
  defp without(keys) do
    endpoint =
      Application.get_env(:pifi, PiFiWeb.Endpoint, [])
      |> Keyword.drop(keys)

    Application.put_env(:pifi, PiFiWeb.Endpoint, endpoint)
  end

  defp endpoint_config, do: Application.get_env(:pifi, PiFiWeb.Endpoint, [])
  ...
  describe "put/1" do
    ...
    test "it keeps the other keys of the endpoint", %{data_dir: data_dir} do
      ...
      assert endpoint_config()[:pubsub_server] == PiFi.PubSub
    end
  end
end
