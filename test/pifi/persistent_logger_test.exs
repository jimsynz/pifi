defmodule PiFi.PersistentLoggerTest do
  use ExUnit.Case, async: true

  alias PiFi.PersistentLogger

  setup do
    directory = Path.join(System.tmp_dir!(), "pifi_log_#{System.unique_integer([:positive])}")
    File.mkdir_p!(directory)
    on_exit(fn -> File.rm_rf(directory) end)

    # `:file` and `:device` are reserved names in an ExUnit context.
    {:ok, log_path: Path.join(directory, "pifi.log"), device_path: Path.join(directory, "pmsg0")}
  end
  ...
  describe "the limit of the file" do
    ...
    test "read/1 gives the older lines first" do
      directory =
        Path.join(System.tmp_dir!(), "pifi_read_#{System.unique_integer([:positive])}")

      File.mkdir_p!(directory)
      on_exit(fn -> File.rm_rf(directory) end)

      path = Path.join(directory, "pifi.log")
  ...
  describe "a place that cannot take a write" do
    test "a log line never stops the firmware", context do
      settings = config(context, %{file: "/no/such/directory/pifi.log"})
