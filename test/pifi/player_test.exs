defmodule PiFi.PlayerTest do
  ...
  # `PiFi.Player` is one process for the whole node, and the settings are rows.
  ...
  describe "the last station" do
    test "a play that fails stores nothing" do
      ...
      assert :ok = PiFi.Player.play(created)
    ...
    test "an item of a source that is not in use selects nothing" do
      created = station(%{})
      Settings.put!("last_item", created.id)
      PiFi.Playback.enable_source(InternetRadio, false)

      assert restarted_state().item == nil

      PiFi.Playback.enable_source(InternetRadio, true)
    end
