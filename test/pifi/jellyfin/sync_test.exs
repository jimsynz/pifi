defmodule PiFi.Jellyfin.SyncTest do
  ...
  defp cache_jobs, do: all_enqueued(worker: PiFi.Playback.Item.Workers.CacheAudio)
  ...
  describe "sync_library" do
    ...
    # `PiFi.Playback.Item` decides which favourites read on to the card, and it must
    ...
    test "a track holds what the player and the card need" do
      ...
      # A song is not an episode. See `PiFi.Playback.FavouriteAudio`.
    ...
    test "a second read writes no second row, and it keeps what a person did" do
      ...
      {:ok, _item} = PiFi.Playback.set_favourite(first)
    ...
    # `PiFi.Jellyfin.Fill` writes this container for an album that names no artist,
    ...
    test "a mark does not hold a row back" do
      ...
      {:ok, _marked} = PiFi.Playback.set_favourite(one)
    ...
    test "it asks for the picture of each container, and none of a track" do
      ...
      asked =
        all_enqueued(worker: PiFi.Artwork.Worker)
  ...
  describe "cache_favourites" do
    ...
    test "it puts one job in the queue for each marked item" do
      ...
      {:ok, _item} = PiFi.Playback.set_favourite(one)
