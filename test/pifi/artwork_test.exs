defmodule PiFi.ArtworkTest do
  ...
  # `PiFi.Cache.used/1` writes no row for an entry of the last hour, and a test of the
  ...
  defp mark_each_read do
    Application.put_env(:pifi, :cache_touch_after_seconds, 0)

    on_exit(fn -> Application.delete_env(:pifi, :cache_touch_after_seconds) end)
  end
  ...
  describe "serve/1" do
    ...
    # interface reads 25 pictures. See `PiFi.Cache.used/1`.
  ...
  describe "the eviction" do
    test "a read of a new picture removes a colder one when the cache is full" do
      ...
      Application.put_env(:pifi, :cache_limit, byte_size(@png) + 1)
    ...
    test "the picture that a person looked at lately stays" do
      ...
      Application.put_env(:pifi, :cache_limit, byte_size(@png) * 2 + 10)
      Process.sleep(5)
      stub("image/jpeg", @jpeg)
