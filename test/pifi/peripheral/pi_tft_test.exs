defmodule PiFi.Peripheral.PiTftTest do
  ...
  test "it starts dark when the device is in standby" do
    ...
    # change. See `PiFi.Peripheral.PiTft.Stmpe610`.
  ...
  # `PiFi.DeviceUi` owns where a person is and sends the whole level. This screen
  ...
  describe "handle_event/2" do
    ...
    test "the charge reaches the view, so the screen can draw it", %{state: state} do
      {:ok, state} =
        PiTft.handle_event(%PiFi.Event.Device.BatteryChanged{percent: 64, low?: false}, state)
  ...
  describe "the pictures that Emerge may read" do
    ...
    test "a thumbnail of the cache draws what its bytes hold", context do
      ...
      by_extension = [
        runtime_paths: [
          enabled: true,
          allowlist: [PiFi.Cache.directory()],
          extensions: [".jpg"]
        ]
      ]
