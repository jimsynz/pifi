defmodule PiFi.Peripheral.PirateAudioTest do
  # `PiFi.Test.RecordingScreen` is a named process, so two of these cannot run at the
  # same time.
  ...
  test "the first frame holds the name that a person gave the device" do
    ...
    on_exit(fn -> KV.put("pifi_device_name", "") end)
  ...
  # level goes away by itself so that the track comes back. See `PiFi.Output.Volume`.
  describe "the level of the output" do
    test "a level that moves goes on the glass and then goes away", %{state: state} do
      {:ok, state} = PirateAudio.handle_event(started(), state)
      RecordingScreen.forget()

      {:ok, state} = PirateAudio.handle_event(volume(40), state)

      assert state.view.volume_percent == 40
      assert frames() == 1

      {:ok, state} = PirateAudio.handle_info(:clear_volume, state)

      assert state.view.volume_percent == nil
      assert frames() == 2
    end

    # **A person who is still moving the level scheduled a later message.** Each event
    # schedules its own, so a message that arrives for a view that holds no level draws
    # nothing.
    ...
    # A card that holds no level, and a control that a person has not turned on, both
    # leave nothing for the person to move.
    test "a level that nothing can set draws nothing", %{state: state} do
      ...
      {:ok, ^state} =
        PirateAudio.handle_event(
          %Player.VolumeChanged{percent: 40, enabled?: false, supported?: true},
          state
        )
  ...
  describe "the network" do
    test "a router that goes off reaches the screen", %{state: state} do
      ...
      {:ok, state} = PirateAudio.handle_event(down(), state)
    ...
    # A second event that says the same thing draws nothing, because the view did not
    # move. VintageNet publishes for each address that an interface takes.
    ...
    defp down, do: %DeviceEvents.NetworkChanged{interfaces: [%{connection: :disconnected}]}
    defp up, do: %DeviceEvents.NetworkChanged{interfaces: [%{connection: :internet}]}
  end
  ...
  describe "the buttons" do
    test "a press says which button, and never what it means", %{state: state} do
      PiFi.Event.subscribe(:input)

      {:ok, _state} = tap_button(state, 5)

      assert_receive %PiFi.Event.Input.ButtonPressed{
        peripheral: PiFi.Peripheral.PirateAudio,
        button: 1
      }
    end
    ...
    test "the second line is the second button", %{state: state} do
      ...
      assert_receive %PiFi.Event.Input.ButtonPressed{button: 2}
    end
    ...
    test "a line that this board does not read says nothing", %{state: state} do
      ...
      refute_receive %PiFi.Event.Input.ButtonPressed{}, 200
    end
  end
