defmodule PiFi.Output.VolumeTest do
  ...
  setup do
    ...
    :ok = PiFi.Player.select_output(@first)
  ...
  describe "a player that is busy" do
    test "the level answers while the player answers nothing" do
      ...
      after
        :sys.resume(PiFi.Player)
      end
    end
  end

  # The card in use changes when a person chooses another one, and no uevent says so.
  # `PiFi.Output.Volume` keeps the card that this event names, so a firmware that
  ...
  describe "a person who chooses another card" do
    # The second card of `PiFi.Test.TwoCardOutput` holds no level, in the way that the
