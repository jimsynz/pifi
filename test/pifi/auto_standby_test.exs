defmodule PiFi.AutoStandbyTest do
  ...
  # holds a minute of 10 ms. See `PiFi.AutoStandby.start_link/1`.
  @minute_ms 10
  ...
  # `PiFi.Application` starts none of these in the test environment, so a test gets the
  # one that it asks for and no other. See `PiFi.Application.listening_children/0`.
  ...
  describe "the period of quiet" do
    ...
    test "a press of a button starts the period again" do
      ...
      Enum.each(1..20, fn _ ->
        Event.publish(:input, %Input.ButtonPressed{peripheral: PiFi.Peripheral.PiTft, button: 2})
    ...
    test "a person who uses a web page starts the period again" do
      ...
      # that they are there. See `PiFiWeb.Shell`.
      Enum.each(1..20, fn _ ->
        Event.publish(:input, %Input.PageUsed{page: PiFiWeb.BrowseLive})
  ...
  describe "the setting" do
    ...
    # `PiFi.Playback.Player` names the process for the whole node, so the action needs
