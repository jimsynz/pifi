defmodule PiFi.PlayerControlsTest do
  @moduledoc """
  The controls of the player: pause, next, previous, and skip.

  `PiFi.Source.InternetRadio` and `PiFi.Source.Podcasts` hold their own tests of
  ...
  defmodule Episodes do
    ...
    @spec episodes() :: [PiFi.Playback.Item.t()]
    def episodes do
      for number <- 1..3 do
    ...
    def holds(options) do
      Application.put_env(:pifi, :episodes_options, options)
    end

    def forget, do: Application.delete_env(:pifi, :episodes_options)

    defp option(name, default) do
      Application.get_env(:pifi, :episodes_options, []) |> Keyword.get(name, default)
    end
  end
  ...
  setup do
    ...
    SilentOutput.use_it()
    Application.put_env(:pifi, :sources, [Episodes])
    Episodes.forget()
  ...
  describe "the history of what a person heard" do
    ...
    # rule must stay empty. See `PiFiWeb.HistoryLive`.
  ...
  describe "a skip" do
    ...
    # `PiFi.Player.AdtsFrame` reads AAC, so a track of that codec moves in the same
    ...
    # `PiFi.Player.FlacFrame` bisects the file, because a FLAC header names the
    ...
    # `PiFi.Player.Pipeline.decoder_holds_stream?/1`.
    ...
    # `PiFi.Player.Skip.frames/1` names the codecs that this firmware reads the
    ...
    # A live stream has no file to move inside, and `PiFi.Player.FileSource` is the
    # element that moves.
