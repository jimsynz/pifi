defmodule PiFi.Test.NoCardOutput do
  @moduledoc """
  An output that finds no sound card.

  `PiFi.Output.Alsa` lists every card of the machine, so the host of a developer
  ...
  Use it with `PiFi.Test.NoCardOutput.use_it/0`, which puts the configuration
  ...
  def sink_spec(device_id) do
    raise "PiFi.Test.NoCardOutput holds no card, and #{device_id} cannot play."
  end
