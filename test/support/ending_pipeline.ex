defmodule MyHiFi.Test.EndingPipeline do
  @moduledoc """
  A pipeline that plays nothing and then says that the track ended.

  `MyHiFi.Player.Pipeline` holds a sink, the sink holds `aplay`, and `aplay` holds
  ...
  `MyHiFi.Player` reads, so the player cannot tell the difference: the audio began,
  and then it ended.

  Use it with `MyHiFi.Test.EndingPipeline.use_it/0`.
  """
  ...
  def use_it do
    ...
    ExUnit.Callbacks.on_exit(fn -> Application.delete_env(:my_hi_fi, :pipeline) end)
  end
