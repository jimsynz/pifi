defmodule PiFiWeb.ArtworkControllerTest do
  ...
  # `PiFi.Artwork.generate_thumbnail/1` writes on the device.
  ...
  describe "show" do
    ...
    test "gives 404 for a name that is not a hash", %{conn: conn} do
      ...
      write("pifi.db", "the database")

      conn = get(conn, ~p"/artwork/pifi.db")
