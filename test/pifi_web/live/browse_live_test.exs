defmodule PiFiWeb.BrowseLiveTest do
  ...
  setup do
    ...
    # and these tests read the page of one. See `PiFi.Source.enabled?/1`.
    for module <- PiFi.Source.all(), do: PiFi.Source.enable(module, true)

    # `PiFi.Player` is one process for the whole node, so its state outlives a test.
    PiFi.Player.stop()
    on_exit(fn -> PiFi.Player.stop() end)
    :ok
  end
  ...
  defp changed(view) do
    Event.publish(:source, %Event.Source.Changed{source: PiFi.Source.Podcasts, ref: :show})
    render(view)
  end
  ...
  # and a bookmark keep it. See `PiFiWeb.BrowseLive`.
  ...
  # who presses this wants the track after what is on now. See `PiFiWeb.ItemList`.
  ...
  describe "playing a track" do
    ...
    # `PiFiWeb.PlayerLiveTest` covers the drawing of it. This page keeps the flash for
  ...
  describe "a device in standby" do
    ...
    # `PiFiWeb.BrowseLive` draws no collection while it says that this firmware holds
    ...
    test "a page that draws no list stays alive when a source changes", %{conn: conn} do
      ...
      # `PiFi.Source.chosen/0` gives the first source in use for every other state.
      for module <- PiFi.Source.all(), do: PiFi.Source.enable(module, false)
  ...
  describe "the control that brings the filters and the sort" do
    ...
    # listing. See `t:PiFi.Source.listing/0`.
    #
    # **The table sorts by its headers**, so the word sits in one of those. A grid holds
    # no header and Cinder draws a control for it instead. See `PiFiWeb.BrowseLive`.
    ...
    # `PiFiWeb.ItemList.filter_title/2`.
    ...
    # after every Z. See `PiFi.Playback.Item`.
  ...
  # reach it. See `PiFiWeb.BrowseLive.letters/1`.
  ...
  # that answer alone. See `PiFi.Source.refresh/2`.
  describe "the control that reads a source again" do
    ...
    test "it asks the source, and it tells the person", %{conn: conn} do
      ...
      # is waiting. `PiFi.Event.Source.Changed` brings the newer episodes.
      refute_enqueued(worker: PiFi.Podcast.Show.Workers.Refresh)
  ...
  # See `PiFiWeb.ItemList.playlist_sheet/1` and `PiFiWeb.PlaylistLive`.
  ...
  describe "the source switch" do
    ...
    test "a source that goes out of use gives the first source in use", %{conn: conn} do
      ...
      on_exit(fn -> PiFi.Source.enable(PiFi.Source.Podcasts, true) end)
  ...
  test "a firmware with no source says so", %{conn: conn} do
    Application.put_env(:pifi, :sources, [])
  ...
  describe "the picture of a collection" do
    ...
    # it with no read of the cache. See `PiFi.Artwork.thumbnail_path/1`.
    test "a row of a container draws its picture", %{conn: conn} do
      {_artist, album} = collection_tree()
      path = PiFi.Artwork.thumbnail_path(album.artwork_url)
  ...
  describe "the head of a collection" do
    test "it shows the name, the words and the picture of the collection", %{conn: conn} do
      ...
      assert html =~ PiFi.Artwork.thumbnail_path(album.artwork_url)
    end
    ...
    # it, which is the rule that the play control follows. See `PiFiWeb.ItemList`.
