defmodule PiFiWeb.PlaylistLiveTest do
  # A playlist plays through `PiFi.Playback.Queue`, which is on ETS and not private,
  # so its rows outlive one test in the way that `PiFi.Player` does.
  ...
  describe "the list of playlists" do
    ...
    # the rule that a row of a list follows. See `PiFiWeb.ItemList`.
  ...
  # each one. See `PiFiWeb.QueueLive`.
