defmodule PiFiWeb.QueueLiveTest do
  # `PiFi.Playback.Queue` is on ETS and the table is not private, so its rows outlive
  # one test in the way that `PiFi.Player` does.
  ...
  describe "a queue that holds rows" do
    ...
    test "an event of the player draws the queue again", %{conn: conn, rows: rows} do
      ...
      PiFi.Event.publish(:player, %PiFi.Event.Player.Stopped{reason: :requested})
    ...
    test "an event that moves nothing draws nothing again", %{conn: conn, rows: rows} do
      ...
      PiFi.Event.publish(:player, %PiFi.Event.Player.Progress{position_ms: 1000})
