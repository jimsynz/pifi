defmodule PiFiWeb.ShellTest do
  # The pages read `PiFi.Player`, which one process holds for the whole node.
  ...
  describe "what a person does on a page" do
    test "a page that a person loads sends one event", %{conn: conn} do
      ...
      assert_receive %Input.PageUsed{page: PiFiWeb.BrowseLive}

      # `PiFiWeb.Layouts` renders the faceplate inside the page, and the router
    ...
    test "a control of the faceplate names the faceplate", %{conn: conn} do
      ...
      assert_receive %Input.PageUsed{page: PiFiWeb.PlayerLive}
    end
  end
  ...
  defp stub_artwork do
    ...
    on_exit(fn -> Application.delete_env(:pifi, Artwork) end)
