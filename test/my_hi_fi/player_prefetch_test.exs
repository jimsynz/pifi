defmodule MyHiFi.PlayerPrefetchTest do
  @moduledoc """
  ...
  defmodule Library do
    ...
    @spec track(String.t()) :: MyHiFi.Playback.Item.t()
    def track(ref) do
  ...
  # The task of `MyHiFi.Player.Prefetch` reads the file, so a test waits for it rather
  ...
  describe "reading one track early" do
    test "the file is on the card, and no mark holds it against an eviction" do
      ...
      # `MyHiFi.Player.release_file/1` for the fault that this repeats otherwise.
