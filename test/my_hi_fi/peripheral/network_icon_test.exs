defmodule MyHiFi.Peripheral.NetworkIconTest do
  use ExUnit.Case, async: true

  alias MyHiFi.Peripheral.NetworkIcon

  doctest MyHiFi.Peripheral.NetworkIcon
  ...
  describe "connection/1" do
    ...
    test "an interface that reaches nothing gives the state that it holds" do
      assert NetworkIcon.connection([%{connection: :disconnected}]) == :disconnected
    end
    ...
    test "a device that names no interface gives nothing" do
      assert NetworkIcon.connection([]) == nil
    end
    ...
    test "a state that this module does not name reads as disconnected" do
      assert NetworkIcon.connection([%{connection: :something_else}]) == :disconnected
    end
  end
  ...
  describe "render/2" do
    ...
    test "a network that carries the music draws nothing" do
      assert NetworkIcon.render(:internet) == NetworkIcon.render(nil)
      assert NetworkIcon.render(:internet) == none()
    end
    ...
    test "the two faults draw different marks" do
      assert NetworkIcon.render(:lan) != NetworkIcon.render(:disconnected)
      refute NetworkIcon.render(:lan) == none()
      refute NetworkIcon.render(:disconnected) == none()
    end

    # A screen names its own size, and the mark of a larger one is not the mark of a
    # smaller one.
    test "the height is the caller's" do
      assert NetworkIcon.render(:lan, height: 20) != NetworkIcon.render(:lan, height: 11)
    end
  end
