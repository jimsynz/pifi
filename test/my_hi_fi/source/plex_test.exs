defmodule MyHiFi.Source.PlexTest do
  ...
  # `MyHiFi.Player.Hls` reads a playlist through a Req of its own, so a test of the
  # conversion gives that one a stub and not the stub of the server.
  defp stub_hls(fun) do
    Application.put_env(:my_hi_fi, Hls, plug: {Req.Test, Hls}, retry: false)
    on_exit(fn -> Application.delete_env(:my_hi_fi, Hls) end)
  ...
  describe "finding the server" do
    ...
    test "naming the server starts the read of the library" do
      ...
      assert_enqueued(worker: MyHiFi.Plex.Sync.Workers.Library)
    end
  ...
  describe "resolve/1" do
    ...
    # Ogg carries Vorbis and it carries FLAC, and `MyHiFi.Player.Pipeline` builds a
