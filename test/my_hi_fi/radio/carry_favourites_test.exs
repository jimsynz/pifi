defmodule MyHiFi.Radio.CarryFavouritesTest do
  use MyHiFi.DataCase, async: false

  alias MyHiFi.Playback
  alias MyHiFi.Radio.CarryFavourites
  alias MyHiFi.Radio.Fill

  # `MyHiFi.Radio.Station` is gone and a migration drops its table, so this makes the
  ...
  test "a marked station becomes a marked item" do
    ...
    assert CarryFavourites.run(MyHiFi.Repo) == 1
  ...
  test "it carries nothing when no station is marked" do
    ...
    assert CarryFavourites.run(MyHiFi.Repo) == 0
  ...
  test "the next fill writes the real fields and keeps the mark" do
    ...
    CarryFavourites.run(MyHiFi.Repo)
  ...
  test "a station that the fill already wrote takes the mark" do
    ...
    assert item.source_ref == "kept"
    assert item.title == "The title of the service"
    assert Ash.count!(MyHiFi.Playback.Item) == 1
  end
  ...
  test "a device with no old table carries nothing" do
    MyHiFi.Repo.query!("DROP TABLE stations", [], log: false)
