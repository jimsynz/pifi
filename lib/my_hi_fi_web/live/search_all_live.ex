defmodule MyHiFiWeb.SearchAllLive do
  @moduledoc """
  Find an item in every source that a person left in use.

  `MyHiFiWeb.SearchLive` searches one source and draws one collection. This page draws
  ...
  `c:MyHiFi.Source.search_groups/1` names the groups, so this page holds no knowledge
  ...
  **This page reads on a submit, and not on each letter.** A group costs a count and a
  ...
  # identifier, with no branch above it, in the way that `MyHiFiWeb.SearchLive` does.
  #
