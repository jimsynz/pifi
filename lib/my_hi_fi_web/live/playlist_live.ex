defmodule MyHiFiWeb.PlaylistLive do
  @moduledoc """
  The lists that a person made.

  Two views live here. `:index` draws every playlist, and `:show` draws the tracks of
  one. `MyHiFiWeb.QueueLive` is the page that this one follows: a row of tracks, a
  ...
  **The queue is what fills the first one.** A person plays an album or a country and
  ...
  track adds it from the list that they found it in. See `MyHiFiWeb.ItemList`.
  ...
  **A playlist of nothing plays nothing**, and the page says so rather than starting a
  ...
  draws a whole playlist. That is the difference from `MyHiFiWeb.QueueLive`, which
  ...
  # `MyHiFiWeb.QueueLive` follows the same rule for the same reason.
  @impl Phoenix.LiveView
  ...
  # one for each row. See `MyHiFiWeb.QueueLive`, which follows the same two rules: one
