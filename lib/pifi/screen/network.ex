defmodule PiFi.Screen.Network do
  @moduledoc """
  ...
  the rule that `PiFi.Peripheral` names. It is `PiFi.Screen.Battery` for the
  network.
  ...
  **A screen draws this only when the network cannot carry the music.** A person whose
  ...
  `connection/1` reduces what `PiFi.Device.network!/0` gives to the one state that
  ...
  The reason is the one that `PiFi.Screen.Battery` gives. Emerge draws an SVG
  ...
  **A mark of one colour and an exclamation mark beside it was the try before this.** It
  ...
  - `:height` - the height of the tallest bar, in pixels. #{@default_height} by default,
    which is the height of the body of `PiFi.Screen.Battery`, so the two sit
    level.
  """
  ...
  @doc """
  The state of the device, from what `PiFi.Device.network!/0` gives.
  ...
  **A device has more than one interface**, and Wi-Fi and a cable both count, so the
  best state of any of them is the state of the device.
      ...
      iex> PiFi.Screen.Network.connection([%{connection: :lan}, %{connection: :internet}])
      :internet

      iex> PiFi.Screen.Network.connection([])
      nil
  """
