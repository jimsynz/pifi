defmodule PiFi.Radio.Fill do
  @moduledoc """
  Write the stations of Radio Browser into the catalogue.

  A station becomes one `PiFi.Playback.Item`, and its country, its language, its
  tags and its bitrate become `PiFi.Playback.Facet` rows that the item links to.
  ...
  `PiFi.Source.InternetRadio.resolve/1` reads an HLS playlist and the first page of
  ...
  Each entry is what `PiFi.Radio.RadioBrowser` gives.
  """
  ...
  # one.** Nothing on the path of a page asks: `PiFi.Artwork.thumbnail_path/1` builds
  # an address and reads nothing, and the address of a picture that the cache does not
  # hold answers 404. `PiFi.Jellyfin.Fill` asks for the same reason.
  #
  # A country of 247 stations therefore asks for 247 logos, and one query decides which
  # of them the cache already holds. A logo is small, the eviction of `PiFi.Cache`
  ...
  # that container. `PiFi.Source.InternetRadio.resolve/1` finds the true one.
