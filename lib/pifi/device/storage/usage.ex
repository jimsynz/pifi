defmodule PiFi.Device.Storage.Usage do
  @moduledoc """
  ...
  **A download belongs to the source that gave it, and the cache cannot say which.**
  `PiFi.Cache` keeps every episode and every track in the one namespace `download`,
  ...
  **`other` covers what is left, and it is large.** A measurement on 2026-09-09 gave
  ...
  # colour moved. The sources come first, in the order of `PiFi.Source.all/0`, and
  # `other` comes last because it is the fold of what no kind names.
  #
  # `PiFiWeb.SettingsLive` names a colour for each key of this list, and the two must
