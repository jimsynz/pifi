...
# hold no reference to this module. See `PiFi.Setup.Monitor` for the same pattern.
if Mix.target() != :host do
  defmodule PiFi.Device.Monitor do
    @moduledoc """
    ...
    `PiFi.Event.Device` on the `:device` topic when an answer changes.
    ...
    - **The cache is the only part that writes a file that stays**, so a write there is
      the one thing that moves the free space of the card. `PiFi.Cache.Entry` names
      `Ash.Notifier.PubSub`, and this process is the one subscriber of it. A download of
      an episode grows outside the cache and `PiFi.Cache.put_file/3` moves it in when
      it is whole, so the free space settles at the moment of that notification.
      `PiFi.Device.Storage.Report` reads `df` and not `:disksup`, because `:disksup`
    ...
    process is the second to last child of `PiFi.Application`, so it starts about six
    ...
    def init(_options) do
      ...
      :ok = Phoenix.PubSub.subscribe(PiFi.PubSub, @cache_topic)
