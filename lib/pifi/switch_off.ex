defmodule PiFi.SwitchOff do
  ...
  **A device that runs on a battery has no way to turn its own power off.** The
  ...
  and say so. `PiFi.Event.Device.SafeToSwitchOff` is what says it, and the screen
  draws it.
  ...
  **`synchronous: :full` is the other answer to this, and it is the wrong one here.** It
  ...
  3. **Put the database on the card.** `PiFi.Cache.Touches` writes the used marks
  ...
  **The fsync comes last on purpose.** It is the barrier: everything that any step
  ...
  because `PiFi.Cache.put_file/3` renames it only when it is whole.
  `PiFi.Player.Download.sweep/0` takes such a file at the next boot.
  ...
  def marker, do: Application.get_env(:pifi, :switch_off_marker, @marker)
  ...
  **A device that no person changed does not**, because the device on a stereo is the
  ...
  A page or a person at the console can call this. `PiFi.Playback.standby/1` reaches
