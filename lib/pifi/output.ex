defmodule PiFi.Output do
  @moduledoc """
  Where the audio goes.

  An output lists the hardware that it can find, and it returns a Membrane sink for
  one piece of that hardware. `PiFi.Player` therefore needs no knowledge of any
  particular device.

  `PiFi.Output.Alsa` is the only output today, and it lists every sound card that
  ...
  **A DAC of a stereo often has none, and that is not a fault.** The PCM5102A of a
  ...
  `PiFi.Output.Volume` owns the number, because the hardware forgets it at each boot
  ...
  `PiFi.Player` reads this each time that it needs a sink, so a change of the
  configuration needs no restart. A test sets `:output` to give an output of its
  own, in the same way that `PiFi.Source.all/0` reads `:sources`.
  """
  @spec module() :: module()
  def module, do: Application.get_env(:pifi, :output, PiFi.Output.Alsa)
end
