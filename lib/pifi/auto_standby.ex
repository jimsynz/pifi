defmodule PiFi.AutoStandby do
  ...
  with an automatic standby does. `PiFi.Playback.set_standby_minutes/1` sets the
  ...
  **A low battery is the second reason to enter standby**, and it needs no period at all.
  ...
  it. See `PiFi.Peripheral.Battery` and `PiFi.Peripheral.ActivityLed`.

  A control of a person starts the period again, whether the control is a button of
  the board or a click of the web page. Both reach `PiFi.Player`, and the player
  ...
  **A person at a browser reaches the player never on most pages**, and the period ran
  out while they read a list. `PiFiWeb.Shell` therefore publishes
  `PiFi.Event.Input.PageUsed` for each event of each page, and that event starts the
  period again as a press of a button does.

  ## Why this is a process of its own

  `PiFi.Player` keeps the standby state already, so the timer looks like it belongs
  ...
  This process keeps no copy of what the player does. It reads `PiFi.Playback.state`
  ...
  def init(options) do
    ...
    # lost. See `PiFi.Device.Monitor`, which learnt this the hard way.
  ...
  # reads. See `PiFi.Peripheral.Battery`.
  #
