defmodule PiFi.Artwork.Worker do
  @moduledoc """
  ...
  `PiFi.Artwork` answers from the cache.

  ## One job for a list, and not one job for each picture

  **A read of a library asks for thousands of pictures at one time.**
  `PiFi.Jellyfin.Fill` asks for the picture of each container that it writes, and one
  ...
  `PiFi.Artwork.ensure/1` therefore writes one job for a batch of addresses, in one
  ...
  **A job tells the `:player` topic only when the caller asks it to.** The player asks
  ...
  that logo with no reload. `PiFiWeb.PlayerLive` draws whatever such an event names,
  ...
  **A picture that a person is waiting for goes first.** The player asks for the logo of
  ...
  # `PiFi.Playback.Item` is in it: a person who marked an album would then wait for
  ...
  # matters. `PiFi.Artwork.ensure/1` holds that rule instead: it asks for no address
  ...
  `PiFi.Artwork.ensure/1` reads it to cut a list into jobs.
  """
  ...
  other caller leaves it alone, and `PiFi.Artwork.ensure/1` is what a list of
  ...
  `PiFi.Artwork.ensure/1` takes the addresses that the cache does not hold and that
  ...
  `PiFi.Artwork.ensure/1` reads this of each job that waits, so a read of a library
  ...
  # `PiFi.Artwork.fetch/1` answers such an address from the cache.
  defp read(urls) do
    urls
  ...
  defp generate_thumbnail(name) do
    with {:ok, entry} <- PiFi.Cache.fetch("artwork", name) do
