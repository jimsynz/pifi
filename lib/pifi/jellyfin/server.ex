defmodule PiFi.Jellyfin.Server do
  @moduledoc """
  ...
  the internet. `PiFi.Settings` keeps the address, the token and the identifier of
  ...
  **`DeviceId` must stay the same for the life of the device.** The server lists one
  ...
  arrives**, because `PiFi.Player.Pipeline` builds a decoder from it. A list of
  ...
  `PiFi.Jellyfin.Fill` therefore writes `format` of each track from the container
  ...
  **The token travels in the query and not in a header.**
  `PiFi.Player.Download` builds the headers of its own request, and it takes none
  ...
  # `PiFi.Jellyfin.Fill` builds from those all exist together, so this number decides
  ...
  @typedoc "One artist, one album or one track, in the shape that `PiFi.Jellyfin.Fill` takes."
  @type entry :: %{
  ...
  the three can reach nothing. `PiFi.AutoSync` asks this before it reads a
  library.
  """
  ...
  reads pages until it has them all. See `PiFi.Jellyfin.Sync.Library`.
  ...
  name gives `nil` there, and `PiFi.Jellyfin.Fill` writes it under the artist that
  it keeps for those.
  """
  ...
  # `PiFi.Playback.FavouriteAudio` reads that size before it asks for a track. It
  ...
  # `PiFi.Jellyfin.Fill.genre_key/0`.
  ...
  # it, and the `artwork` calculation of `PiFi.Playback.Item` does that already.
  ...
  # `PiFi.Playback.FavouriteAudio` reads this size before it asks for a track, and
  ...
  # date.** `PremiereDate` is the release, and `PiFi.Playback.Item` keeps that under
  ...
  # A test gives a stub with `config :pifi, PiFi.Jellyfin.Server, plug: ...`,
  # in the same way that `PiFi.Podcast.Index` takes one. Nothing sets this in
  # production.
