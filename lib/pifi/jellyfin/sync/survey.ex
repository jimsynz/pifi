defmodule PiFi.Jellyfin.Sync.Survey do
  @moduledoc """
  Asks a Jellyfin server what changed before reading anything.
  ...
  **A read of a whole library is 80 minutes and tens of thousands of writes**, and it
  ...
  This asks three cheap questions — one request each, reading a single entry so that the
  answer carries `TotalRecordCount` — and gives `PiFi.Jellyfin.Sync.Library` one of three
  answers.
  ...
  **A library that gained one record and lost another counts the same.** The walk would
  ...
  read, so `PiFi.Jellyfin.Sync.Library` runs one on a schedule of its own whatever this
  ...
  **An edit is invisible too.** A person who fixes the spelling of an album changes no
  ...
  The count says how many are new and not which. `DateCreated` descending puts the
  newest first, so the walk reads pages until it meets a reference that the catalogue
  holds, and everything before that is new. It stops at `@limit` references, because a
  library that gained thousands is one where reading the lot is the cheaper answer
  anyway.

  **A read of 67,508 items took 88 minutes** and ran every day whether a person had
  added a record or not. That is the number this exists to avoid paying.
  """
  ...
  @typedoc "What a survey found, and what `PiFi.Jellyfin.Sync.Library` should do about it."
  ...
  The kinds that a survey counts, in the order that a read of them must happen.
      ...
      iex> PiFi.Jellyfin.Sync.Survey.kinds()
  ...
  """
  @spec take(map()) :: finding()
  def take(link) do
    case counted(link) do
      {:ok, counts} -> weigh(counts, link)
      :error -> :everything
    end
  end
  ...
  defp counted(link) do
    Enum.reduce_while(@kinds, {:ok, %{}}, fn kind, {:ok, counts} ->
      case Server.count(kind, link) do
  ...
  defp walked(link) do
    Enum.reduce_while(@kinds, {:added, %{}}, fn kind, {:added, found} ->
      case newest_until_known(kind, 0, [], link) do
  ...
  defp newest_until_known(kind, start, found, link) do
    case Server.page(kind, start, link, newest: true) do
      {:ok, %{count: 0}} ->
        {:ok, found}
      ...
      {:ok, %{entries: entries, count: count, total: total}} ->
        ...
        cond do
          ...
          true -> newest_until_known(kind, start + count, found, link)
        end
  ...
  # is a track. This is the same split that `PiFi.Source.Jellyfin` browses by.
  ...
  # `PiFi.Jellyfin.Fill.unknown_artist_ref/0`.
