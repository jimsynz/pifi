defmodule PiFi.Jellyfin.Sync.Checkpoint do
  @moduledoc """
  ...
  - `started_at` is the time that the **first** read of this pass began.
    `PiFi.Jellyfin.Sync.Library.remove_unseen/1` removes each row that the pass did
  ...
  It is one row of a key and a value, and `PiFi.Settings` already has that shape.
