defmodule PiFi.Jellyfin.Sync.Favourites do
  @moduledoc """
  ...
  moment reads nothing. `PiFi.AutoSync` runs this when the source is in use, the
  ...
  **The rule that decides what reads names no source at all.**
  `PiFi.Playback.FavouriteAudio` reads `transport` and `keeps_place?` of an item,
