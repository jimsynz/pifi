defmodule PiFi.Player.Download do
  @moduledoc """
  ...
  the pacing and `PiFi.Player.HttpSource` needs none of its own. A podcast server
  ...
  and `PiFi.Player.FileSource` reads that file while it grows. A file needs no
  flow control.
  ...
  `PiFi.Cache.put_file/3`, which moves it. **Both paths must sit on one
  partition**, because that move is `File.rename/2`.

  The file waits outside the cache while it grows, because
  `PiFi.Cache.Entry.Changes.Write` names a row that names a file that exists. A
  ...
  **A stop of the playback does not end it.** The request is already in flight, and
  ...
  files from filling the partition, and `PiFi.Application` calls it at each boot.
  ...
  **The request carries a range, so a read resumes.** A server that closes the
  ...
  defp request(%State{} = state, from) do
    ...
    path = state.path
    uri = state.uri
    options = Application.get_env(:pifi, __MODULE__, [])
  ...
  # See the `:coldest` read of `PiFi.Cache.Entry`.
  ...
  # `PiFi.Event.Player.Progress` keeps the same period for the same reason.
