defmodule PiFi.Player.Pipeline do
  @moduledoc """
  The Membrane pipeline that plays one stream.

  Three facts of `PiFi.Source.playable/0` build it: the transport, the
  ...
  # A podcast episode is a file that `PiFi.Player.Download` writes as fast as the
  ...
  # container itself. See `PiFi.Player.PortDecoder`.
  ...
  # demultiplexer sets. See `PiFi.Player.MpegAudio`.
  defp adapter(link, %{transport: :hls, container: :mpeg_ts, format: :mp3}) do
    child(link, :mpeg_audio, PiFi.Player.MpegAudio)
  end
  ...
  #
  # A live stream hid this. `PiFi.Player.HttpSource` keeps its own ring buffer for
  ...
  # An Ogg stream needs nothing. `PiFi.Player.HttpSource` gives a
  ...
  **A program that decodes through a port reads one stream from its own beginning.**
  ...
  `PiFi.Player` therefore builds the pipeline again for such a codec, and
  `PiFi.Player.FileSource` does the skip before a byte leaves it. A decoder of
  Membrane finds the next frame by itself, so MP3 and AAC keep the pipeline and the
  reader moves inside it.

  The list is the one that `decoder/2` below sends to `PiFi.Player.PortDecoder`.
  """
  ...
  # work through a port. See `PiFi.Player.PortDecoder`. Each program reads the
