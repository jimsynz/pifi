defmodule PiFi.Player.Mp3Frame do
  @moduledoc """
  ...
  **The byte that a resume starts at is in front of what a person heard.**
  `PiFi.Player.FileSource` reports the byte that it read, and the pipeline runs a
  ...
  **A bitrate cannot turn a time into a byte.** 11 of the 46 episodes of the
  ...
  for a file of one. `PiFi.Player.Skip` says what a backward skip then does with
  that measurement.
  ...
  A backward skip meets the same wall, and `PiFi.Player.Skip` answers it in the
  ...
  `limit` is the byte to stop at. `PiFi.Player.FileSource` gives the count that the
  ...
  **The answer is an estimate.** A file of one bitrate lands exactly, and a file of
  many lands near. `PiFi.Player.Skip` therefore measures the span that it chooses,
