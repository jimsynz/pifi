defmodule PiFi.Player.Prefetch do
  @moduledoc """
  ...
  and once for the first 64 KB that `PiFi.Player.FileSource` waits for before the first
  ...
  **It does not make the change of track gapless by itself.** `PiFi.Output.APlayPort`
  ...
  next. `PiFi.Output.APlayPort.wrote_first/0` measures that silence, and
  `[:pifi, :player, :gap]` carries it.
  ...
  **One track ahead.** `PiFi.Playback.Queue.NextUp` gives the row after the one
  ...
  `PiFi.Player.Download` writes each file with `keep?`, which no eviction may take.
  ...
  a stopped song did before `PiFi.Player.release_file/1`.

  ## Why a task, and not the player

  `PiFi.Player.Download.ensure/2` makes its caller a watcher, so every count of
  ...
  """
  @spec ask(PiFi.Playback.Item.t()) :: :ok | :ignored
  def ask(item) do
