defmodule PiFi.Player.FileSource do
  @moduledoc """
  A Membrane source that reads audio from a file while that file grows.

  `PiFi.Player.Download` writes the file as fast as the network allows, and this
  ...
  that another library owns, which is why `PiFi.Player.HttpSource` dropped the
  ...
  `PiFi.Player.Download` and this element make a resume exact: the reader knows
  ...
  # person heard noise. `PiFi.Player.HttpSource` never met this, because a part of
  ...
  def_options(
    ...
    format: [
      ...
      `PiFi.Player.Skip.frames/1` says which codecs carry frames that this
      firmware reads.
      """
    ],
    skip_ms: [
      ...
      it runs**, so `PiFi.Player` builds the pipeline again and gives the skip
  ...
  # `PiFi.Player.HttpSource`, where that cost a build to find.
  ...
  # about one second. See `PiFi.Player.Skip`.
  #
  ...
  # direction says which of the two paths did it. See `PiFi.Player.Skip`.
  defp placed(%State{} = state, ms) do
    metadata = %{direction: direction(ms), format: state.format}

    :telemetry.span([:pifi, :player, :skip], metadata, fn ->
  ...
  `PiFi.Player.Mp3Frame` for both reasons.

  This function is public so that a test can reach it, as `PiFi.Player.HttpSource`
  ...
  # this element, so the decoder reads one stream and never two. `PiFi.Player`
  # builds the pipeline again for a codec whose decoder holds the state of the
  # stream. See `PiFi.Player.Pipeline.decoder_holds_stream?/1`.
  #
  ...
  # header of `channels: 0` and `bits: 0` and `PiFi.Player.PortDecoder` stopped on
  # it. `PiFi.Player.FlacFrame.header/1` gives the 42 bytes that answer it, and
