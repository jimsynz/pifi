defmodule PiFi.Plex.Companion.Router do
  @moduledoc """
  ...
  Every command becomes a call of `PiFi.Playback`, so a person who presses pause on
  ...
  firmware is `PiFi.Playback.Queue`, which is a list on the device and not a play
  ...
  # controller sends `shuffle` and `repeat` here as well, and `PiFi.Playback.Queue`
  # holds neither yet.
  ...
  the wrong machine.
      ...
      iex> PiFi.Plex.Companion.Router.level_of("/player/playback/stepForward")
      :info

      iex> PiFi.Plex.Companion.Router.level_of("/library/metadata/470959")
      :debug
  ...
  **It takes the state and it reads none**, so a test may give it a device that plays a
  ...
  defp metadata_of("1", %{item: %{source_ref: ref}, source: PiFi.Source.Plex})
  ...
  # `PiFi.Playback` holds none of those.
  ...
  # lives in memory, so this costs no read of the card. See `PiFi.Playback.Queue`.
  defp moving do
  ...
  defp server(%{source: PiFi.Source.Plex}) do
  ...
  # `PiFi.Playback.Queue` lives in memory, so it goes when the power does, and
  # `PiFi.Player` restores the one track that a person left. The queue of that device
  ...
  defp from_queue(%{refs: refs, selected: selected} = queue) do
    ...
    case ids do
      ...
      ids ->
        ...
        # that it was given. See `PiFi.Plex.Companion.Queue`.
        Queue.keep(queue)
