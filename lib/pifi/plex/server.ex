defmodule PiFi.Plex.Server do
  ...
  audio. `PiFi.Settings` keeps the address, the tokens and the identifier of the
  ...
  **The codec and the container are two facts, and the pipeline needs both.** MP3,
  ...
  and `PiFi.Plex.Fill` writes both from what the server reports.
  ...
  **Everything else is a conversion**, and `transcode_url/2` is the address of it. A
  ...
  # sound and no error.
  #
  # MP3 is the one to name. `PiFi.Player.Pipeline` builds `:hls, :mpeg_ts, :mp3` for
  # 8 of the New Zealand stations already, and `PiFi.Player.MpegAudio` holds the one
  ...
  # `PiFi.Player.Hls.resolve/2` takes this and the answer is not a guess.
  ...
  # `PiFi.Plex.Fill` builds from those all exist together, so this number decides the
  # memory of the whole read, on a board with 363.9 MB.
  #
  # 50 is the number that `PiFi.Jellyfin.Server` measured, and the two reads build the
  ...
  @typedoc "One artist, one album or one track, in the shape that `PiFi.Plex.Fill` takes."
  @type entry :: %{
  ...
  two can reach nothing. `PiFi.AutoSync` asks this before it reads a library.
  """
  ...
  `PiFi.Plex.Companion.Router` names these to a controller and the headers of each
  ...
  **This must not ask for a strong code.** The `strong` parameter of that endpoint
  ...
  the same as no setting**: `PiFi.Settings` takes no empty value, so this removes the
  row.
  """
  ...
  artists. A server with none gives an empty list, and `PiFi.Plex.Sync.Library`
  then writes nothing.
  """
  ...
  reads pages until it has them all. See `PiFi.Plex.Sync.Library`.
  ...
  **Plex converts for a client that it holds a profile for, and for no other.** A
  ...
  MPEG-TS that carry MP3. `PiFi.Player.Hls` reads the master and gives the media
  playlist, and `PiFi.Player.Pipeline` already builds `:hls, :mpeg_ts, :mp3` for 8 of
  the New Zealand stations.

  **Every value is in the query, and no header takes part.** A measurement on a board
  on 2026-09-14 gave 200 for that, which is what lets `PiFi.Player.Hls.resolve/2`
  ...
  **A master playlist of Plex names no `CODECS`**, so `PiFi.Player.Hls.resolve/2`
  ...
  not name gives `nil` there, and `PiFi.Plex.Fill` writes it under the artist that
  it keeps for those.
  ...
  # `PiFi.Plex.Fill` writes the two of them through one path for that reason. A server
  ...
  track with no part cannot play, and `PiFi.Source.Plex.resolve/1` says so.
  """
  ...
  pages makes one read and not three for each one. See `PiFi.Plex.Sync.Library`.
  ...
  **This is not `client_id/0`.** That one names this device, and this one names the
  ...
  # the `artwork` calculation of `PiFi.Playback.Item` does that already.
  #
  ...
  # drew the mark that `PiFi.Artwork` draws for a picture it does not hold.
  #
  # **This costs a read of the artwork again when a person links the device again.**
  # `PiFi.Artwork` keeps each picture under the SHA-256 of its address, so a new
  # token is a new address and no cached picture answers it. `artwork_url` is one of
  # the `upsert_fields` of `PiFi.Plex.Fill`, so the next read of the library writes
  # the new address and the pictures arrive again over the local network. A link again
  # is a rare thing, and the alternative is for `PiFi.Artwork` to hold knowledge of
  ...
  # date.** `year` is the release, and `PiFi.Playback.Item` keeps that under
  ...
  # A test gives a stub with `config :pifi, PiFi.Plex.Server, plug: ...`, in the
  # same way that `PiFi.Jellyfin.Server` takes one. Nothing sets this in production.
