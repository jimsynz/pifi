defmodule PiFi.Plex.Sync.Checkpoint do
  @moduledoc """
  ...
  - `started_at` is the time that the **first** read of this pass began.
    `PiFi.Plex.Sync.Library.remove_unseen/1` removes each row that the pass did not
  ...
  **`section` is the one part that `PiFi.Jellyfin.Sync.Checkpoint` has no need of.**
  ...
  It is one row of a key and a value, and `PiFi.Settings` already has that shape. A
  ...
  **It raises nothing.** A point that no write reached costs a read of the library
  ...
  @doc "The name that `PiFi.Settings` keeps this under."
  @spec key() :: String.t()
  def key, do: @key
