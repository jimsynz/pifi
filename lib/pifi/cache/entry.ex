defmodule PiFi.Cache.Entry do
  ...
  sqlite do
    ...
    # `PiFi.Artwork.serve_thumbnail/1` loads `variants`, which reads
    ...
    custom_indexes do
      ...
      # `PiFi.Cache.bytes/0` sums this column over every row, and without an index
  ...
  actions do
    ...
    create :put do
      ...
      change PiFi.Cache.Entry.Changes.Write
    end
    ...
    create :put_file do
      ...
      when the file is whole. See `PiFi.Player.Download`.
      """
    ...
    create :put_from_url do
      ...
      them itself. See `PiFi.Cache.Entry.Changes.Fetch`.
      """
      ...
      change PiFi.Cache.Entry.Changes.KeyFromUrl
      change PiFi.Cache.Entry.Changes.Fetch
      change PiFi.Cache.Entry.Changes.Write
    end
  ...
  # episode grows outside the cache and `PiFi.Cache.put_file/3` moves it in when it is
  # whole, so the free space settles at that moment.
  #
  # `PiFi.Device.Monitor` is the one subscriber, and it turns this into
  # `PiFi.Event.Device.StorageChanged` on the `:device` topic. A part of this firmware
  # reads that typed event and never this one. See `PiFi.Event`.
  pub_sub do
    # `module` is the module that defines `broadcast/3`, and `name` is the process that
    # runs the pub sub. `PiFi.PubSub` is a name and not a module, so it belongs in the
    ...
    # firmware owns, so `PiFi.Artwork` reads the one topic and decides which of its
    # own entries arrived. See `PiFi.Artwork.subscribe/0`.
  ...
  changes do
    change PiFi.Cache.Entry.Changes.PurgeVariants, on: [:destroy]
  end
  ...
  relationships do
    has_many :attachments, PiFi.Cache.Attachment do
      description """
