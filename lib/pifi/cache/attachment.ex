defmodule PiFi.Cache.Attachment do
  @moduledoc """
  ...
  relationship and filters for its own type. See `PiFi.Podcast.Show`.
  ...
  **A record that goes therefore leaves its rows behind.** A host that removes itself
  ...
  attributes do
    ...
    attribute :record_type, :string do
      ...
      A string and not an atom, for the reason that `PiFi.Cache.Entry` gives for its
      namespace.
      """
  ...
  relationships do
    belongs_to :entry, PiFi.Cache.Entry do
