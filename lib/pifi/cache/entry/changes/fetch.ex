defmodule PiFi.Cache.Entry.Changes.Fetch do
  @moduledoc """
  Reads an address and gives the bytes to `PiFi.Cache.Entry.Changes.Write`.
  ...
  anything. `PiFi.Artwork` reads the first bytes of an image for that reason.
  ...
  # A test gives a stub with `config :pifi, PiFi.Cache, plug: ...`. Nothing
  ...
  defp get(url) do
    ...
    |> Keyword.merge(Application.get_env(:pifi, Cache, []))
    |> Req.new()
    |> Req.get()
    |> case do
