defmodule PiFi.Output.Volume do
  @moduledoc """
  ...
  **The setting keeps the number, and the card is only told it.** A read of a card
  ...
  `PiFi.Output.put_volume/2` is a write and never a read.
  ...
  writes it again whenever `PiFi.Event.Device.OutputChanged` says that the card in
  use changed. `PiFi.Player` could hold it, and it answers a control in more than a
  dozen clauses, so a write in each one is a write that the next clause forgets. That is
  the reason that `PiFi.AutoStandby` is a process as well.
  ...
  **A reader of the level never waits for this process and never dies with it.** Every
  ...
  `PiFi.Playback.Player.state/0`, and the reason is the same.
  ...
  # card in use is a call to `PiFi.Player`, and that process is busy for as long as a
  ...
  #
  # `PiFi.Event.Device.OutputChanged` names the card in use, and it arrives for a card
