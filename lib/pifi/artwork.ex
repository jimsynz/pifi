defmodule PiFi.Artwork do
  ...
  `PiFi.Cache` keeps the file and the row, in the namespace `"artwork"`. This module
  ...
  **A person also gives a picture, and it lives here too.** The splash of the device
  ...
  reader of this module therefore takes it. See `PiFi.Device.Identity`.
  """
  ...
  # The topic that `PiFi.Cache.Entry` publishes each new entry of this namespace on.
  # See `subscribe/0`.
  ...
  reload. See `PiFiWeb.Shell`.

  `PiFi.Cache.Entry` publishes the notification, so this is one subscription for a
  page and never one for a row.
  """
  @spec subscribe() :: :ok | {:error, term()}
  def subscribe, do: Phoenix.PubSub.subscribe(PiFi.PubSub, @ready_topic)
  ...
  the picture arrives with no address to ask for again.
      ...
      iex> PiFi.Artwork.ready(%Ash.Notifier.Notification{data: %PiFi.Cache.Entry{}})
      :error
  ...
  **It reads nothing.** The name of an entry is the hash of the address, so this builds
  ...
  `PiFi.Artwork.Worker.enqueue/1` costs a write for each row that the cache misses.
  `PiFiWeb.ItemList` draws a list again for each event of the player, so that cost
  ...
  picture, and a row of a container shows a folder until the picture arrives.
      ...
      iex> PiFi.Artwork.thumbnail_path(nil)
      nil
  ...
  something used the entry, which is what orders the eviction. See `PiFi.Cache`.

  The entity tag is the checksum of the bytes, which `PiFi.Cache` writes for each
  ...
  **One read for the whole list.** `PiFi.Artwork.Worker.enqueue/2` reads the cache
  ...
  **One insert for the whole list as well.** A read of a library gives thousands of
  ...
  `PiFi.Artwork.Worker.enqueue_all/1` writes one job for each 100 addresses.
  ...
  **The entry stays against every eviction.** No address can read these bytes again, so
  ...
  `PiFi.Playback.Item.RemoveCache`.
  ...
  **4 of the 247 New Zealand stations name the text `"null"` as their logo**,
  ...
  `PiFi.Artwork.Worker` then failed three times for a station that can never hold
  a picture.
  ...
  `PiFi.Device.Storage.Usage` reads it, so the storage report names the same
  namespace that this module writes. See `PiFi.Player.Download.namespace/0`.
  """
  ...
  """
  @spec path(PiFi.Cache.Entry.t()) :: Path.t()
  ...
  writes a new one over it. See `PiFi.Artwork.Thumbnail.digest/0`.
  """
  @spec generate_thumbnail(PiFi.Cache.Entry.t()) ::
          {:ok, PiFi.Cache.Entry.t()} | {:error, term()}
  ...
  """
  @spec thumbnail(PiFi.Cache.Entry.t()) :: PiFi.Cache.Entry.t() | nil
  ...
  `PiFi.Artwork.Thumbnail` writes it when it makes the thumbnail, so this reads a
  row and no picture. The device screen and the web page both read this, and neither
  one has a rule of its own. See `PiFi.Artwork.Accent`.
  ...
  `PiFi.Artwork.Thumbnail` is one thing that moves it. The address of a thumbnail
  ...
  # The count comes first, because `PiFi.Cache.purge_all/1` reports that it worked
  ...
  # See the `:put` action of `PiFi.Cache.Entry` for why the variant action of
  # `AshStorage` cannot.
  ...
  # A test gives a stub with `config :pifi, PiFi.Artwork, plug: ...`. Nothing
