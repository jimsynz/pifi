defmodule PiFi.Player do
  @moduledoc """
  Plays one track at a time.

  A caller gives one `PiFi.Playback.Item`, and the player asks the source of it to
  resolve it, builds a pipeline, and plays it. It runs one pipeline at a time, and it
  stops the old one first.

  `PiFi.Playback.play/2` is what a page calls. It puts the list that a person saw in
  `PiFi.Playback.Queue` and then calls this with the row that they pressed.
  ...
  **A play, a skip, and a move all answer at once, and the work follows in a
  ...
  `buffering/1` publishes `PiFi.Event.Player.Buffering` with that answer, so every
  ...
  **A fault of that work is an event, and not the answer of the call.** `fail/2`
  publishes `PiFi.Event.Player.Failed`, and the player bar of the web interface and
  ...
  A skip keeps the pipeline. It moves the byte that `PiFi.Player.FileSource` reads,
  because a pipeline owns the decoder and the buffer of a stream, and a start of one
  reads the network again. `PiFi.Output.APlayPort` keeps the sound card across a
  pipeline now, so a start no longer opens the card unless the format of the audio
  changed. See `PiFi.Player.Skip`.

  Next and previous move the mark of `PiFi.Playback.Queue`, which decides the order
  ...
  below give one. `PiFiWeb.Telemetry` draws each of them at `/dev/dashboard`.

  - `[:pifi, :player, :resolve, :start | :stop | :exception]` covers
    `c:PiFi.Source.resolve/1`, which reads the service behind the page.
  - `[:pifi, :player, :sound]` holds the time from the call to the first sound.
    **This is the wait that a person feels**, and it holds the resolve, the first
    bytes, and the moment that `aplay` opens the card.
  - `[:pifi, :player, :track]` holds how long a track sounded, and the reason
    says whether a person stopped it or it reached its end.
  - `PiFi.Player.FileSource` writes
    `[:pifi, :player, :skip, :start | :stop | :exception]`, because the element
  ...
  `PiFi.Playback.play/2` puts the list in the queue and calls this with the row that
  a person pressed. See `PiFi.Playback.Queue`.
  ...
  **This leaves standby**, as `pause/1` and `next/0` do, because a person who asks for
  ...
  """
  @spec play(PiFi.Playback.Item.t()) :: :ok | {:error, term()}
  ...
  `PiFi.Event.Player.Progress` event.
  ...
  `PiFi.Player.Skip` reads MP3 frames alone. A track that makes no sound yet gives
  ...
  `PiFi.Playback.Player.state/0` does that, and every page reads it through there.
  """
  ...
  # `PiFi.Device.Monitor` publishes `PiFi.Event.Device.OutputChanged` for a card
  ...
  # `PiFi.Player.FileSource` reports the byte that it has read. The player keeps
  ...
  defp resolved(source, item) do
    :telemetry.span([:pifi, :player, :resolve], %{source: source}, fn ->
  ...
  # A test names its own pipeline with `config :pifi, :pipeline, ...`, in the
  ...
  defp start_pipeline(playable, sink, _state) do
    module = Application.get_env(:pifi, :pipeline, Pipeline)
  ...
  # the sound. `PiFi.Event.Device.OutputChanged` carries the same three fields, so a
  ...
  # cache does not hold arrives in a later event, from `PiFi.Artwork.Worker`.
  defp artwork_path(%{artwork: url}) do
    case Artwork.name(url) do
      nil ->
        ...
        # the `:player` topic when the picture arrives. See `PiFi.Artwork.Worker`.
  ...
  # that gives the file, and `PiFi.Player.FileSource` is the element that moves. All
  ...
  # whole one. See `PiFi.Player.Pipeline.decoder_holds_stream?/1`.
  #
  # The cost is small now. `PiFi.Output.APlayPort` keeps the sound card across a
  ...
  # `PiFi.Player.FileSource` does the skip before a byte leaves it, and it reports
  ...
  **This reads the settings and not the process**, so it answers while the player is
  busy and it answers before the player has started. `PiFi.Playback.Player.idle/0`
  ...
  # the `:mark_played` action of `PiFi.Playback.Item`.
  #
  ...
  # `PiFi.Player.Prefetch` for what this does not remove.
  #
  ...
  # wait. A cache entry of this track is the answer: `PiFi.Player.Download` writes
  ...
  defp whole?(%{id: id}) do
    match?({:ok, _entry}, PiFi.Cache.fetch(Download.namespace(), id))
  end
