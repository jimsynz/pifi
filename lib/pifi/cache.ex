defmodule PiFi.Cache do
  ...
  `PiFi.Artwork` uses `:artwork` and keys by the hash of an address, and a later
  version uses `:download` and keys by the identifier of an episode.

  `PiFi.Cache.Entry` keeps the data about one entry, and `AshStorage` writes the
  file.
  ...
  resources do
    ...
    resource PiFi.Cache.Attachment do
  ...
  **It writes nothing for a row that already says that it was used inside the hour.**
  ...
  A mark that it does keep goes to `PiFi.Cache.Touches`, which keeps it in memory and
  ...
  `PiFi.Cache.Touches` writes its buffer with this, so one flush is one statement and
  ...
  **This counts at the time of the query, and it is not an aggregate of a resource.**
  ...
  `{:filter_relationship, _}`, which is what these two use.
      ...
      iex> PiFi.Cache.usage_of("show", show.id)
  ...
  `PiFi.Playback.FavouriteAudio` reads this before it asks for a track, because a
  ...
  def directory do
    PiFi.Device.storage!().path |> Path.join(@directory) |> Path.expand()
  end
  ...
  **What the cache already holds is a part of that space, and the count must add it
  ...
  def limit do
    case Application.get_env(:pifi, :cache_limit) do
  ...
  defp free_bytes_of_partition do
    Application.get_env(:pifi, :cache_free_bytes) || PiFi.Device.storage!().free_bytes
  end
  ...
  defp touch_after_seconds do
    Application.get_env(:pifi, :cache_touch_after_seconds, @touch_after_seconds)
  end
end
