defmodule PiFi.Event.Source do
  @moduledoc """
  ...
  one, and each one already knows how to read a container. See `PiFi.Source.browse/2`.
  """
  ...
  defmodule AudioChanged do
    ...
    that. `PiFi.Player.Download` sends this while it reads a file and once when the
    file is whole.
    ...
    **No source sends this one, and it rides this topic on purpose.** What the card
    ...
    part that draws a list already takes the `:source` topic: `PiFiWeb.ItemList`
    ...
    **It arrives at most once a second while a file reads.** The download writes about
    ...
    too fast to read. `PiFi.Event.Player.Progress` holds the same period for the same
    reason.
    """
  ...
  defmodule Changed do
    ...
    @type t :: %__MODULE__{source: module(), ref: PiFi.Source.ref()}
