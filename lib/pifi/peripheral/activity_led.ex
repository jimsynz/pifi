defmodule PiFi.Peripheral.ActivityLed do
  @moduledoc """
  ...
  `PiFi.Event.Device.BatteryChanged` says that the charge is low, and it leaves the
  ...
  **The kernel does the flashing, and this process does not.** `/sys/class/leds/ACT`
  ...
  question that `PiFi.Peripheral.enabled?/1` answers for a screen, so it belongs here.
  ...
  # warning that no Elixir code reads the attribute. See `PiFi.Output.Alsa`, which
  # does the same.
