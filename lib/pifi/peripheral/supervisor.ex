defmodule PiFi.Peripheral.Supervisor do
  @moduledoc """
  Holds the process of each peripheral that runs.

  It starts with no child. `PiFi.Peripheral.start_enabled/0` puts each peripheral
  ...
  **The children are not a static list, and the hardware is the reason.** A child
  ...
  own, and `PiFi.Peripheral.start/1` gives the error of that start to the caller.

  The identifier of each child is the module of the peripheral, so one supervisor
  runs a screen and a knob together, and `PiFi.Peripheral.stop/1` names one of
  them.
  """
