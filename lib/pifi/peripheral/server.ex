defmodule PiFi.Peripheral.Server do
  @moduledoc """
  The process that a peripheral runs in.

  It subscribes to the topics of `c:PiFi.Peripheral.subscriptions/0`, and it calls
  `c:PiFi.Peripheral.handle_event/2` for each event that arrives. A peripheral
  ...
  ## How to start one
      ...
      {PiFi.Peripheral.Server, module: PiFi.Peripheral.PiTft, rotation: :landscape}

  Every option except `:module` and `:name` goes to `c:PiFi.Peripheral.init/1`.
  ...
  A screen turns its backlight off in `c:PiFi.Peripheral.terminate/2`, and a
  ...
  # person presses is one of those. See `PiFi.Event` and `c:PiFi.Peripheral.handle_info/2`.
