defmodule PiFi.Source.Podcasts do
  @moduledoc """
  ...
  subscribes to it. See the `container` type of `PiFi.Source`.

  ## The two services

  The index finds a show, and it returns no episode. The feed of the publisher gives
  the episodes. `PiFi.Podcast.Feed` reads it, and a private feed therefore works
  ...
  A search and the trending list write a `PiFi.Podcast.Show` for each answer, so
  ...
  # `PiFi.Player.Skip` reads MP3 frames, and 8771 of the 8773 episodes of the
  ...
  **A person opening a show wants the episode of this week**, and a feed of a daily
  ...
  # and for an episode of a show that they hold, and `PiFiWeb.SearchLive` draws a
  # control to choose between the two. See `c:PiFi.Source.kinds/0`.
  def search(text) do
    read_index(text)
  ...
  `c:PiFi.Source.hold_limit/0`, and `PiFi.Playback.FavouriteAudio` for what reads
  it.
  """
  @impl PiFi.Source
  def hold_limit do
  ...
  # `PiFi.AutoSync` reads this before it asks for the trending list or for a feed.
  @impl PiFi.Source
  ...
  # `PiFi.Podcast.Trending` publishes `PiFi.Event.Source.Changed` when it finishes,
  # and a page that shows Trending reads the list again.
  @impl PiFi.Source
  def run_settings_action("read_index") do
    case PiFi.Source.ask_for_job(Show, :read_trending) do
      :queued ->
  ...
  # the local copy is old. See the `stale?` calculation of `PiFi.Podcast.Show`.
  #
  ...
  # `PiFi.Event.Source.Changed`, and a page that shows the show reads it again.
  ...
  # A row of `PiFi.Podcast.Show` carries the feed and the index, and an item carries what
  ...
  # `PiFi.Podcast.Fill` reads the type of the enclosure, so this needs no mime type to
  ...
  # nothing to play. `PiFi.Podcast.CarryPlaces` writes one, and the next read of the
  # feed fills it.
  ...
  defp playable(item) do
    {:ok,
     %{
       ...
       # `PiFi.Player.Download` keeps the file under this name. It is the identifier
  ...
  # The subtitle and the picture are columns of the item that `PiFi.Podcast.Fill`
