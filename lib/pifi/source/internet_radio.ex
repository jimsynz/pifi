defmodule PiFi.Source.InternetRadio do
  @moduledoc """
  ...
  `PiFi.Radio.Fill` writes the stations into `PiFi.Playback`, so this module keeps
  no table of its own and it reaches no network to browse. A search works when the
  internet does not.

  A `ref` is `{:station, id}`, and the id is the id of a `PiFi.Playback.Item`. A
  country and a tag are containers, and each one names a `PiFi.Playback.Facet`.
  ...
  # nothing to play. `PiFi.Radio.CarryFavourites` writes one, and the next sync
  ...
  # codec. `PiFi.Player.Hls` reads it. This is the one function of this module
  ...
  def put_settings(%{"countries" => text}) do
    case codes(text) do
      ...
      codes ->
        PiFi.Settings.put!(FromRemote.countries_key(), Enum.join(codes, ","))
  ...
  def run_settings_action("sync") do
    case PiFi.Source.ask_for_job(PiFi.Radio.Sync, :sync_from_remote) do
      :queued ->
