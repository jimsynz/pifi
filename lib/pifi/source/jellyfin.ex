defmodule PiFi.Source.Jellyfin do
  @moduledoc """
  ...
  `PiFi.Jellyfin.Fill` for the shape of the tree, and `PiFi.Jellyfin.Server` for
  the reads.

  ## It browses the catalogue and not the server

  `PiFi.Jellyfin.Sync` copies the library on to the card, so a person browses
  ...
  **The person says when they have typed the code, and no timer does.** A device
  ...
  A song is not an episode of a podcast. `PiFi.Jellyfin.Fill` therefore writes
  ...
  # other Jellyfin client shows them. See `PiFiWeb.CoreComponents` and
  # `assets/vendor/brand.js`.
  @impl PiFi.Source
  def icon, do: :jellyfin

  # **A skip works for every track of this library now.** It was absent while
  # `PiFi.Player.Skip` read MP3 alone: a library of 1088 FLAC tracks and 268 MP3
  # ones would have moved inside one track and not the next, and a control that works
  # for some rows is worse than none. `PiFi.Player.FlacFrame` and
  # `PiFi.Player.AdtsFrame` read the other two codecs that
  # `PiFi.Jellyfin.Server` asks a server for, so the list is whole.
  #
  # **A search reaches no server.** The catalogue holds the whole library, because
  # `PiFi.Jellyfin.Sync.Library` reads every artist, album and track of it, so a
  ...
  # the branch is a plain read of the facets. See `PiFi.Jellyfin.Fill.genre_key/0`.
  ...
  **The order is one column and not two.** `place` of the item carries the disc and the
  ...
  # needs no facet and no column of its own. See `PiFi.Jellyfin.Fill`.
  ...
  # apart.** See `c:PiFi.Source.search_groups/1`.
  @impl PiFi.Source
  ...
  each one. See `PiFi.Playback.FavouriteAudio`.
  """
  ...
  # A device with no link reaches nothing, so `PiFi.AutoSync` writes no job
  # that can only fail.
  @impl PiFi.Source
