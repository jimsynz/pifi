defmodule PiFi.Playback.Item do
  @moduledoc """
  ...
  this table. `PiFi.Source.from_slug/1` reads the name, and it reports a source that
  ...
  the audio, which is the `container` field of `PiFi.Source.playable`. The two are
  ...
  that decide. See `PiFi.Playback.FavouriteAudio`.
  """
  ...
  sqlite do
    ...
    custom_statements do
      ...
      # table nor the index of `parent_id` serves such a comparison. `PiFi.Playback`
      # reads an item by its identifier for every press of this firmware, and
      # `PiFiWeb.BrowseLive` reads the items of a container each time that a person
      # opens one.
      #
      ...
      # both: `PiFi.Playback.get_item/1` took 370 ms and the items of one container
  ...
  oban do
    triggers do
      ...
      # and `PiFi.Jellyfin.Sync.Favourites` is the other caller of it.
      #
      ...
      trigger :cache_audio do
        ...
        worker_module_name PiFi.Playback.Item.Workers.CacheAudio
        queue :default
        max_attempts 1
      end
    end
  ...
  actions do
    ...
    read :marked_for_audio do
      ...
      **The newest mark comes first.** A card that fills stops the run, so the order
      ...
      and it comes last. See `PiFi.Playback.FavouriteAudio`.
      """
    ...
    read :by_ids do
      ...
      **The play queue reads this.** A queue row lives in ETS and an item lives in
      ...
      query per row. See `PiFi.Playback.Queue`.
    ...
    update :mark_started do
      ...
      `PiFi.Player` runs this when the pipeline says that the sound began, and not
      ...
      **This writes one row for each track that plays**, and a track is minutes long, so
      ...
      each restart writes this again, which `PiFi.Player` bounds to five.
      """
    ...
    update :cache_audio do
      ...
      `PiFi.Playback.FavouriteAudio` reads the tracks and writes them to the cache.
      """

      require_atomic? false

      change PiFi.Playback.Item.Changes.CacheAudio
    end
    ...
    update :store_position do
      ...
      reads no file leaves it absent. See `PiFi.Player.FileSource`.
      ...
      `PiFi.Playback.Item.Changes.KeepPlaceOnly`.
      """

      accept [:position_ms, :position_bytes]

      change PiFi.Playback.Item.Changes.KeepPlaceOnly
    end
    ...
    update :mark_played do
      ...
      `PiFi.Player` runs this when a track reaches its end, and a person runs it by
      ...
      change PiFi.Playback.Item.Changes.ReleaseAudio
    end
    ...
    update :clear_played do
      ...
      read of the source: `PiFi.Playback.FavouriteAudio` holds the newest few
    ...
    destroy :destroy do
      ...
      change {PiFi.Cache.Attachment.Changes.DetachRecord, type: "item"}

      # A track that goes takes its audio with it. `PiFi.Player.Download` writes the
      # file with `keep?`, and `:mark_played` and `PiFi.Player.release_file/1` are the
    ...
    action :remove_cache, :integer do
      ...
      against this. See `PiFi.Playback.Item.RemoveCache`.
      """

      argument :source, :string, allow_nil?: false

      run PiFi.Playback.Item.RemoveCache
    end
  end
  ...
  attributes do
    ...
    attribute :source_key, :string do
      ...
      **This is not `url`, and it is not `source_ref`.** `url` is the whole address,
      ...
      `PiFi.Source.Plex` is the one source that uses it. Plex serves the file as
    ...
    attribute :artwork_url, :string do
      description "The address of the picture. `PiFi.Artwork` reads that address, and the cache keeps the file."
      public? true
    end
    ...
    attribute :byte_size, :integer do
      ...
      `PiFi.Playback.FavouriteAudio` reads this before it asks for a track, so it
    ...
    attribute :number, :integer do
      ...
      feed that names none gets none: `PiFi.Podcast.Feed.Parser` keeps 200 episodes
      ...
      the order that its source names. See `c:PiFi.Source.listing/1`.
      """
    ...
    attribute :added_at, :utc_datetime_usec do
      ...
      **It is the date of the service and not of this device.** `inserted_at` says
      ...
      names it `DateCreated`. See `PiFi.Jellyfin.Server.album/2`.
    ...
    attribute :transport, :atom do
      description "How the bytes arrive. See `PiFi.Source.playable`."
    ...
    attribute :favourited_at, :utc_datetime_usec do
      ...
      **`updated_at` cannot answer this.** A sync writes every row that a service
      ...
      `PiFi.Playback.FavouriteAudio` reads the marked items newest first, and this
    ...
    attribute :last_seen_at, :utc_datetime_usec do
      ...
      removes it. See `PiFi.Jellyfin.Sync.Library`.
  ...
  relationships do
    ...
    many_to_many :facets, PiFi.Playback.Facet do
    ...
    # and `PiFi.Player.Download` writes it in the `download` namespace. The cache
    # holds no column that names this resource, so the key and the namespace are what
    # make the join.
    has_one :audio_file, PiFi.Cache.Entry do
      source_attribute :id
    ...
    # the join belong to an item. See `PiFi.Cache.Attachment`.
  ...
  calculations do
    calculate :child_count,
              :integer,
              {PiFi.Playback.GroupedCount, table: "playback_items", column: "parent_id"} do
      description """
    ...
    calculate :caches_audio?,
      ...
      **A container of episodes reads too, and a count is what holds it down.** An
      ...
      `c:PiFi.Source.hold_limit/0` says how many of the newest a source keeps. A
    ...
    calculate :place, :integer, expr((disc || 0) * 1000 + number) do
      ...
      **A sort of two columns cannot do this work.** `Cinder.QueryBuilder` unsets the
      ...
      `PiFiWeb.ItemList.place_text/1` draws the same fact for a person to read, as
      `1-01`.
      """
    ...
    calculate :remaining_ms, :integer, expr(duration_ms - position_ms) do
      ...
      **A live stream and an item of an unknown length give nothing.** `duration_ms` is
      ...
      of one draws no time left. See `c:PiFi.Source.listing/1`.
      """
