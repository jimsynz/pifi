defmodule PiFi.Playback.FavouriteAudio do
  @moduledoc """
  ...
  **A track whose `transport` is `:download`.** That fact of `PiFi.Playback.Item` is
  ...
  - **An episode of a show reads, and a count keeps it down.** A show has hundreds of
    ...
    person has not played**, and `c:PiFi.Source.hold_limit/0` is how many.
  ...
  - A track that keeps its place and that a person marked by itself reads nothing. A
    person marks the show, and `PiFiWeb.ItemList.favourite/1` draws no control on an
  ...
  **A mark reaches two levels.** An album contains tracks, so a mark on an album reads
  ...
  2. **When it has no room, run the ordinary eviction, and name the room that
     this track needs.** That is `PiFi.Cache.prune/1` with `want_bytes`, and it
  ...
  **The eviction of step 2 cannot take what this run has already written.** Each run
  ...
  ever. See the `:coldest` read of `PiFi.Cache.Entry`.

  `PiFi.Playback.Item` gives the marked items newest first, through the
  ...
  card inside its limit, and `PiFi.Cache.prune/0` runs after each track arrives so
  that the next check reads the truth.

  ## The mark holds no file against an eviction

  `PiFi.Player.Download` writes each entry with `keep?`, which no eviction may
  ...
  **`PiFi.Player.Download.ensure/2` returns two answers, and both of them release.**
  ...
  **Nothing here touches an entry that the cache already holds.** `touch` moves the
  ...
  `PiFi.Playback.Item`, and `PiFi.Jellyfin.Sync.Favourites` for the run that
  ...
  `PiFi.Jellyfin.Sync.Favourites`, so this raises nothing.
  ...
  **The link of a source that needs one is read once before the loop.** A source that
  ...
  # `PiFiWeb.BrowseLive` asks for the order of a listing.
  ...
  defp resolve(PiFi.Source.Jellyfin = module, track, links) do
  ...
  defp add_link(slug, acc) do
    case Source.from_slug(slug) do
      {:ok, module} when module == PiFi.Source.Jellyfin ->
        case Server.link() do
  ...
  # message of that download arrives here. See `PiFi.Player.Download`.
