defmodule PiFi.Playback.PlaylistEntry do
  @moduledoc """
  ...
  **A track goes in a playlist twice if a person asks twice.** No identity covers the
  ...
  actions do
    ...
    create :create do
      ...
      `PiFi.Playback.Playlist.Add` is the only caller, because a place that one
    ...
    action :remove, :struct do
      ...
      run PiFi.Playback.PlaylistEntry.Remove
    end
    ...
    action :reorder, :struct do
      ...
      nearest end. See `PiFi.Playback.Playlist.Order`.
      """
      ...
      run PiFi.Playback.PlaylistEntry.Reorder
    end
