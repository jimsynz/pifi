defmodule PiFi.Playback.GroupedCount do
  @moduledoc """
  How many rows of one table point at each record of a page.

  `PiFi.Playback.Facet` uses it to say how many items hold a facet, and
  `PiFi.Playback.Item` uses it to say how many children a container holds. A person
  ...
  **Do not copy this module.** Reach for it, or for `Ash.count/1`. A new place that
  ...
  costs one query. `PiFiWeb.BrowseLive` loads it with `query_opts`, and Cinder keeps
  what that names.
  ...
  defp chunk_counts(ids, opts) do
    ...
    %{rows: rows} = SQL.query!(PiFi.Repo, statement, ids)
  ...
  defp name(given, part) when is_binary(given) do
    if Regex.match?(~r/^[a-z_][a-z0-9_]*$/, given) do
      ...
      {:error, "#{part} of PiFi.Playback.GroupedCount is not a plain name: #{inspect(given)}"}
    end
  end

  defp name(given, part) do
    {:error,
     "#{part} of PiFi.Playback.GroupedCount must be a string, and it is #{inspect(given)}"}
  end
end
