defmodule PiFi.Playback.Item.RemoveCache do
  @moduledoc """
  ...
  **A mark holds nothing against this.** The audio of a favourite waits for an
  ...
  still on the row. See `PiFi.Playback.FavouriteAudio`.
  ...
  **The rows stay, and the files go.** The catalogue is small, and it carries the
  ...
  - **The audio is in `download`, keyed by the identifier of the item.** The join of
    `PiFi.Playback.Item` therefore names each entry, and the number of them is small:
  ...
  - **A picture is in `artwork`, keyed by the hash of its address.** No row of the
    ...
    carry and `PiFi.Artwork.remove_each/1` turns each one into a key. A publisher
  ...
  See `PiFi.Cache.Entry.Changes.PurgeVariants`.
  """
  ...
  # `PiFi.Artwork.remove_each/1` takes each address one time. **Two rows of one album
