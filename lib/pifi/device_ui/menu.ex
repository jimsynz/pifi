defmodule PiFi.DeviceUi.Menu do
  @rows 100
  ...
  `PiFi.DeviceUi` holds where a person is, and this module builds one level at a
  time.
  ...
  ## The tree
      ...
      │   └── Countries            one branch, from `c:PiFi.Source.roots/0`
      │       └── NZ               one facet value
  ...
  **The source names its own branches, so this module knows no source.** A source that
  ...
  and nothing in this file changes. That is the same rule that `PiFiWeb.BrowseLive`
  ...
  **Standby, and nothing else.** The issue that asked for this menu asked for the
  ...
  # The items that carry one facet value. `PiFiWeb.BrowseLive` reads the same query,
