defmodule PiFi.Screen do
  @moduledoc """
  The parts that the screens of this device draw with.

  A peripheral owns its layout. `PiFi.Peripheral.PiTft.Screen` draws 320 by 240
  pixels and `PiFi.Peripheral.PirateAudio.Screen` draws 240 by 240, and the two
  therefore put things in different places. That is the rule that `PiFi.Peripheral`
  ...
  **A part says what a thing looks like, and a layout says where it sits.** The shape
  ...
  - `PiFi.Screen.Badge` puts a mark on a dark band, so it reads over a picture.
  - `PiFi.Screen.Bar` draws a bar that is part full.
  - `PiFi.Screen.Battery` draws the charge of the cell.
  - `PiFi.Screen.Clock` writes the time of a track as words.
  - `PiFi.Screen.Network` draws the state of the network.
  - `PiFi.Screen.Row` puts one thing at each end of a row.
