defmodule PiFi.AutoSync do
  ...
  **The source decides the first two, and a job means nothing without it.** Reading the
  ...
  or without a period. See `PiFi.Source.enabled?/1` and `PiFi.Source.ready?/1`.
  ...
  most. `PiFi.Device.Monitor` measured this: VintageNet reports `:internet` about half
  ...
  for `PiFi.Event.Device.NetworkChanged` alone would therefore sync on the boots where
  Wi-Fi was slow, and on no others.

  ## The gate is cheap on purpose

  `PiFi.Device.Network.Report` holds `signal_percent`, so the report changes whenever
  the strength of the signal moves and `PiFi.Device.Monitor` publishes each change.
  ...
  `PiFi.AutoStandby` does. See `jobs_for/1`.

  A job that never ran holds no time, and it is due at once. That is what fills the
  catalogue of a new device, and it is why `PiFi.Radio.FirstSync` is gone: that module
  ...
  **The time is written before the job goes in the queue.** Two events in one moment
  ...
  # `PiFi.Application` takes each one out of the crontab that `AshOban.config/2`
  ...
  @jobs [
    %{
      ...
      resource: PiFi.Radio.Sync,
      action: :sync_from_remote,
      worker: PiFi.Radio.Sync.Workers.FromRemote,
      default_hours: 168
    },
    %{
      ...
      resource: PiFi.Podcast.Show,
      action: :read_trending,
      worker: PiFi.Podcast.Show.Workers.ReadTrending,
      default_hours: 24
    },
    %{
      key: "podcast-refresh",
      ...
      resource: PiFi.Podcast.Show,
      action: :refresh_all,
      worker: PiFi.Podcast.Show.Workers.RefreshAll,
      default_hours: 6
    },
    %{
      ...
      source: PiFi.Source.Jellyfin,
      resource: PiFi.Jellyfin.Sync,
      action: :sync_library,
      worker: PiFi.Jellyfin.Sync.Workers.Library,
      default_hours: 24
    },
    ...
    # because `PiFi.AutoSync` gates every job on the source that it needs. Jellyfin
    # is the one source today whose tracks read on to the card: a station is live, and
    # an episode of a podcast keeps its place. See `PiFi.Playback.FavouriteAudio`.
    %{
      ...
      worker: PiFi.Jellyfin.Sync.Workers.Favourites,
      default_hours: 1
    },
    %{
      key: "plex-library",
      ...
      resource: PiFi.Plex.Sync,
      action: :sync_library,
      worker: PiFi.Plex.Sync.Workers.Library,
      default_hours: 24
    },
    %{
      key: "plex-favourites",
      ...
      resource: PiFi.Plex.Sync,
      action: :cache_favourites,
      worker: PiFi.Plex.Sync.Workers.Favourites,
      default_hours: 1
    }
  ]
  ...
  it draws `PiFi.Hardware.profiles/0`.
  """
  @spec jobs() :: [map()]
  def jobs, do: @jobs
  ...
  **A job of a source belongs on the page of that source**, and not on a page of its
  ...
  a source that a person turned off or that holds no key. `PiFiWeb.SettingsLive` draws
  ...
  `PiFi.Application` takes each one out of the crontab that `AshOban.config/2` builds,
  ...
  The settings key that holds the period of one job.
      ...
      iex> PiFi.AutoSync.hours_key("radio")
      "sync.radio.hours"
  """
  ...
  The settings key that holds the time that one job last ran.
      ...
      iex> PiFi.AutoSync.last_key("radio")
      "sync.radio.last-run"
  """
  ...
  `PiFi.Device.Monitor` publishes a change of the network, and this runs then. A page
  ...
  def init(options) do
    ...
    # See `PiFi.Device.Monitor`, which learnt this the hard way.
  ...
  # A host holds no VintageNet, so `PiFi.Device.network/0` gives an empty list there
