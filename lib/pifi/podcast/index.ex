defmodule PiFi.Podcast.Index do
  @moduledoc """
  Reads the Podcast Index.

  See <https://podcastindex.org>. The index finds a show, and the feed of the
  publisher gives the episodes. See `PiFi.Podcast.Feed`. Nothing that plays
  ...
  in `PiFi.Settings`, so no firmware image carries a secret and no two devices
  ...
  @typedoc """
  One show, in the shape that `PiFi.Podcast.Show` accepts.
  ...
  private feed is always one of those. `PiFi.Podcast.Feed` reads such a feed
  ...
  def show_by_feed_url(url) do
    case request("/podcasts/byfeedurl", url: url) do
      ...
      # feed is never in the index and `PiFi.Podcast.Feed` reads one itself.
  ...
  @doc """
  Turn one feed of the index into the attributes of `PiFi.Podcast.Show`.
  ...
  # `PiFi.Podcast.Fill` writes each one as a facet.
  ...
  # A test gives a stub with `config :pifi, PiFi.Podcast.Index, plug: ...`,
  # in the same way that `PiFi.Radio.RadioBrowser` takes one. Nothing sets this
  # in production.
  ...
  # No check for a blank key belongs here. `PiFi.Settings.Setting` removes the
