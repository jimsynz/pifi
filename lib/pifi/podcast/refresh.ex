defmodule PiFi.Podcast.Refresh do
  @moduledoc """
  Reads the feed of one show and writes what it gives.

  Two callers need this, and they must not disagree. `PiFi.Source.Podcasts` reads
  a feed when a person opens a show whose local copy is old, and
  `PiFi.Podcast.Show.RefreshAll` reads the feed of each subscribed show on a
  schedule.
  ...
  A read that succeeds publishes `PiFi.Event.Source.Changed`. Both callers run
  ...
  """
  @spec run(PiFi.Podcast.Show.t()) :: PiFi.Podcast.Show.t()
  def run(show) do
  ...
  """
  @spec prune(PiFi.Playback.Item.t()) :: :ok
  def prune(item) do
    Item
  ...
  defp fill(show, attributes, episodes) do
    ...
    # The show and its item meet at this key. Without it `PiFi.Source.Podcasts`
  ...
  # feed changes and this is that moment: `PiFi.AutoSync` runs the refresh of the
  # followed shows on a period, and a person who opens a show refreshes it as well.
  #
  # `PiFi.Playback.FavouriteAudio` decides how many episodes and which ones, and it
