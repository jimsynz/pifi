defmodule PiFi.Podcast.CarryPlaces do
  @moduledoc """
  ...
  This writes an item for each of those, with the identity that `PiFi.Podcast.Fill`
  ...
  **The column list must name every column that takes no null.** A migration runs
  ...
  today. See `PiFi.Radio.CarryFavourites`, which learnt the same lesson.

  ## The file of an episode that a device already read goes

  `PiFi.Player.Download` keys a file by the identifier of the episode, and an item
