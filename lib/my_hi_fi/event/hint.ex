defmodule MyHiFi.Event.Hint do
  @moduledoc """
  ...
  `MyHiFi.DeviceUi` owns the list, so it is the one part that can say.

  **A knob takes this topic and not the `:player` topic.** `MyHiFi.Event.Player.Progress`
  arrives once a second while a track plays, and a knob that woke for it would spend
  the board on an event that it cannot use. See `MyHiFi.Peripheral`.
  """
