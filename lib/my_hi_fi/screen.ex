defmodule MyHiFi.Screen do
  ...
  A peripheral owns its layout. `MyHiFi.Peripheral.PiTft.Screen` holds 320 by 240
  pixels and `MyHiFi.Peripheral.PirateAudio.Screen` holds 240 by 240, and the two
  therefore put things in different places. That is the rule that `MyHiFi.Peripheral`
  holds, and this library does not take it away.
  ...
  **A part says what a thing looks like, and a layout says where it sits.** The shape
  ...
  screen holds a copy. A screen that draws a bar of another height and another colour
  ...
  A part takes a value and its options, and it gives an `t:Emerge.tree/0`. It reads no
  event, it holds no process and it talks to no hardware, so a test draws it to a PNG
  and a person looks at the file.

  **The web interface does not use this library.** A browser holds CSS, and a page
