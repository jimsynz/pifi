defmodule MyHiFi.Playback.Playlist do
  @moduledoc """
  ...
  **A playlist takes a track of any source, because `MyHiFi.Playback.Item` is one
  ...
  `MyHiFi.Playback.Queue` is on ETS, so a restart empties it. A playlist is what a
  ...
  `MyHiFi.Playback.PlaylistEntry` carries the place of each track, and
  `MyHiFi.Playback.Playlist.Order` writes it. `position` counts from 0, and a caller
  cannot make a gap or two entries of one place.

  ## Playing one

  A playlist gives its item identifiers, and `MyHiFi.Playback.play/2` takes them. The
  queue is therefore the one thing that plays, and the player needs no knowledge of a
  playlist at all.

  `MyHiFi.Playback.GroupedCount` gives `entry_count`, because **AshSqlite expresses no
  ...
  actions do
    ...
    action :add, {:array, :struct} do
      ...
      asks for one twice means it. See `MyHiFi.Playback.Playlist.Add`.
      """
      ...
      run MyHiFi.Playback.Playlist.Add
    end
    ...
    action :item_ids, {:array, :uuid} do
      ...
      `MyHiFi.Playback.play/2` takes this list. An entry whose item is gone from the
      catalogue is absent, because there is nothing to play for it.
      """

      argument :playlist_id, :uuid, allow_nil?: false

      run MyHiFi.Playback.Playlist.ItemIds
    end
