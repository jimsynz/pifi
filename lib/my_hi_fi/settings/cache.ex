defmodule MyHiFi.Settings.Cache do
  @moduledoc """
  ...
  **A setting is read many times and written almost never.** A person writes one by
  ...
  the web interface asks `MyHiFi.Source.enabled?/1` for each source, and the player
  asks the same question in three places. Each of those was a `SELECT` on an SD card.

  `MyHiFi.Settings` is the one door to this table, so it is the one place that writes
  ...
  `MyHiFi.Cache.Touches` names the same rule.
  ...
  rollback. `MyHiFi.DataCase` and `MyHiFiWeb.ConnCase` therefore call this before
  each test.
  """
  @spec clear() :: :ok
  def clear do
