defmodule MyHiFi.Hardware.ConfigTxt do
  @moduledoc """
  ...
  lines.
      ...
      # --- MyHiFi hardware profile: begin ---
      dtoverlay=hifiberry-dac
      gpio=25=op,dh
      # --- MyHiFi hardware profile: end ---
  ...
  touches, and `MyHiFi.Hardware` writes this block again when a boot finds it absent.

  ## Every function here is pure

  A read of a file, a mount, and a restart all belong to `MyHiFi.Hardware`. This module
  takes the text and gives the text, so a host can test each rule of it.
  """

  @begin "# --- MyHiFi hardware profile: begin ---"
  @finish "# --- MyHiFi hardware profile: end ---"
  ...
  `MyHiFi.Hardware` reads this at each boot. It compares the text of the file and never
