defmodule MyHiFi.Player.FlacFrame do
  @moduledoc """
  Reads the frames of a FLAC file, to find a place inside it.

  FLAC answers the same two questions that `MyHiFi.Player.Mp3Frame` answers, and it
  ...
  **A decoder that reads no STREAMINFO does not know the shape of a sample.** `flac`
  ...
  header of `channels: 0` and `bits: 0`, and `MyHiFi.Player.PortDecoder` read that
