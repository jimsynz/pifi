defmodule MyHiFi.Player.Ogg do
  @moduledoc """
  Says which codec an Ogg stream holds.
  ...
  holds. Reading from a pipe it cannot go back to the start, so it takes the first
  ...
  # The first page of an Ogg stream holds the identification header, and a page is
  ...
  # This module holds its own configuration key, and it does not share the one of
  # `MyHiFi.Player.Hls`. A shared key made two tests fight over one application
