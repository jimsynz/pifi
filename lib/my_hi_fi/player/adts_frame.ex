defmodule MyHiFi.Player.AdtsFrame do
  @moduledoc """
  Reads the frames of an AAC file in ADTS, to find a place inside it.

  This answers the same two questions for AAC that `MyHiFi.Player.Mp3Frame` answers
  ...
  **An ADTS header names the length of its own frame**, in 13 bits, so a walk forward
  ...
  A Jellyfin server sends AAC in ADTS, because `MyHiFi.Jellyfin.Server` asks for the
  `aac` container. An AAC track inside MP4 holds no ADTS header and never reaches
  here: `MyHiFi.Player.skippable?/1` reads the format of the playable, and a file of
  ...
  # `MyHiFi.Player.Mp3Frame.bytes_of_ms/4`.
  @probe_ms 1000
  ...
  **The answer is an estimate**, in the way that the answer of
  `MyHiFi.Player.Mp3Frame.bytes_of_ms/4` is. `MyHiFi.Player.Skip` measures the span
