defmodule MyHiFi.Source.Plex do
  @moduledoc """
  ...
  the shape of the tree, and `MyHiFi.Plex.Server` for the reads.

  ## It browses the catalogue and not the server

  `MyHiFi.Plex.Sync` copies the library on to the card, so a person browses their music
  ...
  **The person says when they have typed the code, and no timer does.** A device cannot
  ...
  A song is not an episode of a podcast. `MyHiFi.Plex.Fill` therefore writes
  ...
  `MyHiFi.Plex.Server.transcode_url/2` is the address of it. A sample of one real
  ...
  **This is the one path of this source that makes a server do work**, and it is a
  ...
  other. `MyHiFi.Plex.Fill` writes `:unknown` for such a track, and `:hls` as its
  ...
  # other Plex client shows them. See `MyHiFiWeb.CoreComponents` and
  # `assets/vendor/brand.js`.
  @impl MyHiFi.Source
  def icon, do: :plex

  # **A skip works for most of a library of this source, and not for all of it.**
  # `MyHiFi.Player.Skip` reads the frames of MP3 and of AAC in ADTS, and it bisects a
  ...
  **The order is one column and not two.** `place` of the item carries the disc and the
  ...
  # no facet and no column of its own. See `MyHiFi.Plex.Fill`.
  ...
  one. See `MyHiFi.Playback.FavouriteAudio`.
  """
  ...
  # A device with no link reaches nothing, so `MyHiFi.AutoSync` writes no job that can
  # only fail.
  @impl MyHiFi.Source
  ...
  # named their server wants their music, and `MyHiFi.AutoSync` would reach this within
  ...
  defp took(server, servers) do
    ...
    MyHiFi.Source.ask_for_job(Sync, :sync_library)
  ...
  # `MyHiFi.Plex.Fill` writes `:unknown` for such a track, and this is the one path of
  # this source that asks a server to do work. See `MyHiFi.Plex.Server.transcode_url/2`.
  ...
  # The master playlist names one variant, and `MyHiFi.Player.Hls` reads it and gives
