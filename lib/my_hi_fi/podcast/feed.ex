defmodule MyHiFi.Podcast.Feed do
  @moduledoc """
  ...
  It passes each option to `MyHiFi.Podcast.Feed.Parser.new/1`, so `:max_items`
  belongs here.
  ...
  # A test gives a stub with `config :my_hi_fi, MyHiFi.Podcast.Feed, plug: ...`, in
  # the same way that `MyHiFi.Radio.RadioBrowser` takes one. Nothing sets this in
  # production.
  #
