defmodule MyHiFi.Plex.Sync.Library do
  @moduledoc """
  Copies the library of a Plex server into the catalogue.

  `MyHiFi.AutoSync` runs this when the source is in use, the device has a link, and
  ...
  album. `MyHiFi.Plex.Fill` reads the parent of each page out of the catalogue, so the
  ...
  **A Plex server holds a section for each library, and each kind reads every music
  ...
  `MyHiFi.Plex.Sync.Checkpoint` therefore keeps the kind, the section and the offset
  ...
  **A mark does not hold a row back, and neither does a file on the card.** A person
  ...
  alias MyHiFi.Plex.Sync.Checkpoint
  alias MyHiFi.Source
  ...
  # nothing. See `MyHiFi.Source.enabled?/1`.
  @impl true
