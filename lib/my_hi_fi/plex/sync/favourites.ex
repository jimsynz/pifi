defmodule MyHiFi.Plex.Sync.Favourites do
  @moduledoc """
  ...
  reads nothing. `MyHiFi.AutoSync` runs this when the source is in use, the device has
  ...
  **The rule that decides what reads names no source at all.**
  `MyHiFi.Playback.FavouriteAudio` reads `transport` and `keeps_place?` of an item, and
