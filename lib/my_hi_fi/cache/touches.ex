defmodule MyHiFi.Cache.Touches do
  @moduledoc """
  The used marks of the cache, held in memory and written together.

  The eviction of `MyHiFi.Cache` takes the entry that something used least recently,
  ...
  **Nothing that any caller can read.** The three callers of `MyHiFi.Cache.used/1`
  ignore what it answers, and the one reader of `last_accessed_at` is the eviction,
  which `MyHiFi.Cache` describes as least recently used and approximate. An
  ...
  **A debounce alone can wait for ever.** The timeout starts again with each message,
  ...
  - `MyHiFi.SwitchOff.prepare/1`, before it folds the log into the database. A device
    that says that a hand may reach the switch must hold nothing in memory.
  - `MyHiFi.Cache.Entry.Prune`, because the eviction is the one reader of these marks.
    A prune that read a stale row could take the entry whose picture is on a screen.

  **A firmware that runs no buffer writes at once.** `MyHiFi.Cache.used/1` reads
  `Process.whereis/1` and writes the row itself when this process is absent, so the
  test environment needs no buffer and no flush. See `MyHiFi.Application`.
  """
